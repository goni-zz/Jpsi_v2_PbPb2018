...
void doSimultaneousV2MassFit_weight_pt6p5_7p5_cent20_120_HFDown_up(
    ...
    )
{
  ...
  fFinal = new TFile(Form("../../Macros/2021_04_22/roots/2DFit_%s/Final/nominal/2DFitResult_%s_%sw_Effw%d_Accw%d_PtW%d_TnP%d.root", DATE.Data(), kineLabel.Data(), "PR", fEffW, fAccW, isPtW, isTnP));
  ...
  gSystem->mkdir(Form ("figs/EventSelection_up/%s",DATE.Data()), kTRUE);
  gSystem->mkdir(Form("roots/EventSelection_up/%s",DATE.Data()), kTRUE);
  ...
  /*
     if (PR==2) {TString fitRestxt = Form("../roots/EventSelection_up/SimFitResult_%s_%s.txt",bCont.Data(),kineLabel.Data());}
     else if (PR==0 || PR==1) {TString fitRestxt = Form("../roots/EventSelection_up/SimFitResult_%s_%s_ctau_%.2f.txt",bCont.Data(),kineLabel.Data(),ctauCut);}
  ...
  pad2->Draw();
  //c_mass_v2->Update();
  if (ctauCut==0) c_mass_v2->SaveAs(Form("figs/EventSelection_up/%s/v2Mass_%s_ctau_%.5f_%.5f_%s_HFDown.pdf",DATE.Data(),kineLabel.Data(),ctauLow,ctauHigh,cutName.Data()));
  else if(ctauCut==-1)c_mass_v2->SaveAs(Form("figs/EventSelection_up/%s/v2Mass_%s_ctau_%.5f_%s_HFDown.pdf",DATE.Data(),kineLabel.Data(),ctauLow,cutName.Data()));
  else if(ctauCut==1)c_mass_v2->SaveAs(Form("figs/EventSelection_up/%s/v2Mass_%s_ctau_%.5f_%s_HFDown.pdf",DATE.Data(),kineLabel.Data(),ctauHigh,cutName.Data()));
  else if(ctauCut==2)c_mass_v2->SaveAs(Form("figs/EventSelection_up/%s/v2Mass_%s_Inc.pdf",DATE.Data(),kineLabel.Data()));
  //if (PR==0||PR==1) {c_mass_v2->SaveAs(Form("../figs/EventSelection_up/v2Mass_%s_%s_ctau_%.2f.pdf",bCont.Data(), kineLabel.Data(),ctauCut));}
  /*
     Double_t xmass[200];
  ...
  */
  TFile *wf;
  if (ctauCut==0) {wf = new TFile(Form("roots/EventSelection_up/%s/SimFitResult_%s_Eff%d_Acc%d_PtW%d_TnP%d_ctau_%.5f_%.5f_%s_HFDown.root", DATE.Data(),kineLabel.Data(),fEffW,fAccW,isPtW,isTnP,ctauLow,ctauHigh,cutName.Data()),"recreate"); wf->cd();}
  else if(ctauCut==-1) {wf = new TFile(Form("roots/EventSelection_up/%s/SimFitResult_%s_Eff%d_Acc%d_PtW%d_TnP%d_ctau_%.5f_%s_HFDown.root", DATE.Data(), kineLabel.Data(),fEffW,fAccW,isPtW,isTnP,ctauLow,cutName.Data()),"recreate"); wf->cd();}
  else if(ctauCut==1) {wf = new TFile(Form("roots/EventSelection_up/%s/SimFitResult_%s_Eff%d_Acc%d_PtW%d_TnP%d_ctau_%.5f_%s_HFDown.root", DATE.Data(), kineLabel.Data(),fEffW,fAccW,isPtW,isTnP,ctauHigh,cutName.Data()),"recreate"); wf->cd();}
  //  else if (PR==0 || PR==1) {TFile *wf = new TFile(Form("../roots/EventSelection_up/SimFitResult_%s_%s_ctau_%.2f_HFDown.root",bCont.Data(), kineLabel.Data(), ctauCut),"recreate"); wf->cd();}
