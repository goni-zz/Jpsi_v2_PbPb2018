...
void compute_sys_diff_HFDw_10_60()
{
  ...
  for(int ib=0; ib<nPtBinPR; ib++){
    ...
    if(ib<2) fileAltPR = dirAlt+Form("EventSelection_HFDw_v2_pt%.1f-%.1f_y1.6-2.4_muPt0.0_centrality20-120.root",PtBinArrPR[ib],PtBinArrPR[ib+1]);
    else fileAltPR = dirAlt+Form("EventSelection_HFDw_v2_pt%.1f-%.1f_y0.0-2.4_muPt0.0_centrality20-120.root",PtBinArrPR[ib],PtBinArrPR[ib+1]);
  ...
  for(int ib=0; ib<nPtBinNP; ib++){
    ...
    if(ib<1) fileAltNP = dirAlt+Form("EventSelection_HFDw_v2_pt%.1f-%.1f_y1.6-2.4_muPt0.0_centrality20-120.root",PtBinArrNP[ib],PtBinArrNP[ib+1]);
    else fileAltNP = dirAlt+Form("EventSelection_HFDw_v2_pt%.1f-%.1f_y0.0-2.4_muPt0.0_centrality20-120.root",PtBinArrNP[ib],PtBinArrNP[ib+1]);
  ...
  TFile *wf = new TFile("EventSelection_HFDw_sys_10_60.root","recreate");
  wf->cd();
  hPtSys_PR->Write();
  hPtSys_NP->Write();
  //for(int ib=0; ib<nPtBinPR; ib++){
