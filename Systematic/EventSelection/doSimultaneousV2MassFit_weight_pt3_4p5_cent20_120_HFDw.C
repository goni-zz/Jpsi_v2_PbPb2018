...
void doSimultaneousV2MassFit_weight_pt3_4p5_cent20_120_HFDw(
    int ctauCut=-1,
    float ptLow =  3.0, float ptHigh = 4.5,
    ...
    )
{
  ...
  //TFile* rf = new TFile(Form("/home/deathold/work/CMS/analysis/Dwsilon_v2/upsilonV2/plots/MassV2_190506/Dws_%s.root",kineLabel.Data()),"read");
  ...
  TFile *rf;
  if(ctauCut==0) rf = new TFile(Form("../../roots/HFDw_210928/v2mass_hist/%s/Jpsi_%s_Eff%d_Acc%d_PtW%d_TnP%d_ctau_%.5f_%.5f_%s.root",
        DATE.Data(),kineLabelTot.Data(),fEffW,fAccW,isPtW,isTnP,ctauLow,ctauHigh,cutName.Data()),"read");
  else if(ctauCut==-1) rf = new TFile(Form("../../roots/HFDw_210928/v2mass_hist/%s/Jpsi_%s_Eff%d_Acc%d_PtW%d_TnP%d_ctau_%.5f_%s.root",
  ...
  else if(ctauCut==2) rf = new TFile(Form("../../roots/HFDw_210928/v2mass_hist/%s/Jpsi_%s_Eff%d_Acc%d_PtW%d_TnP%d_Inc.root",
  ...
  cout << "****************************************" << endl;
  cout << "	" << "	Lower Limit" << "	" << "Dwper Limit" << endl;
  ...
  pad2->Draw();
  //c_mass_v2->Dwdate();
  if (ctauCut==0) c_mass_v2->SaveAs(Form("figs/v2mass_fit_210929/%s/v2Mass_%s_ctau_%.5f_%.5f_%s_HFDw.pdf",DATE.Data(),kineLabel.Data(),ctauLow,ctauHigh,cutName.Data()));
  else if(ctauCut==-1)c_mass_v2->SaveAs(Form("figs/v2mass_fit_210929/%s/v2Mass_%s_ctau_%.5f_%s_HFDw.pdf",DATE.Data(),kineLabel.Data(),ctauLow,cutName.Data()));
  else if(ctauCut==1)c_mass_v2->SaveAs(Form("figs/v2mass_fit_210929/%s/v2Mass_%s_ctau_%.5f_%s_HFDw.pdf",DATE.Data(),kineLabel.Data(),ctauHigh,cutName.Data()));
  ...
  */
  TFile *wf;
  if (ctauCut==0) {wf = new TFile(Form("roots/v2mass_fit_210929/%s/SimFitResult_%s_Eff%d_Acc%d_PtW%d_TnP%d_ctau_%.5f_%.5f_%s_HFDw.root", DATE.Data(),kineLabel.Data(),fEffW,fAccW,isPtW,isTnP,ctauLow,ctauHigh,cutName.Data()),"recreate"); wf->cd();}
  else if(ctauCut==-1) {wf = new TFile(Form("roots/v2mass_fit_210929/%s/SimFitResult_%s_Eff%d_Acc%d_PtW%d_TnP%d_ctau_%.5f_%s_HFDw.root", DATE.Data(), kineLabel.Data(),fEffW,fAccW,isPtW,isTnP,ctauLow,cutName.Data()),"recreate"); wf->cd();}
  else if(ctauCut==1) {wf = new TFile(Form("roots/v2mass_fit_210929/%s/SimFitResult_%s_Eff%d_Acc%d_PtW%d_TnP%d_ctau_%.5f_%s_HFDw.root", DATE.Data(), kineLabel.Data(),fEffW,fAccW,isPtW,isTnP,ctauHigh,cutName.Data()),"recreate"); wf->cd();}
