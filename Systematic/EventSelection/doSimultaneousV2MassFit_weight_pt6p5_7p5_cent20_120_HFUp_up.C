...
void doSimultaneousV2MassFit_weight_pt6p5_7p5_cent20_120_HFUp(
    ...
    )
{
  ...
  TFile *rf;
  if(ctauCut==0) rf = new TFile(Form("../../roots/v2mass_hist/%s/Jpsi_%s_Eff%d_Acc%d_PtW%d_TnP%d_ctau_%.5f_%.5f_%s_HFUp.root",
        DATE.Data(),kineLabelTot.Data(),fEffW,fAccW,isPtW,isTnP,ctauLow,ctauHigh,cutName.Data()),"read");
  else if(ctauCut==-1) rf = new TFile(Form("../../roots/v2mass_hist/%s/Jpsi_%s_Eff%d_Acc%d_PtW%d_TnP%d_ctau_%.5f_%s_HFUp.root",
  ...
  gSystem->mkdir(Form ("figs/EventSelection/%s",DATE.Data()), kTRUE);
  gSystem->mkdir(Form("roots/EventSelection/%s",DATE.Data()), kTRUE);
  ...
  /*
     if (PR==2) {TString fitRestxt = Form("../roots/EventSelection/SimFitResult_%s_%s.txt",bCont.Data(),kineLabel.Data());}
     else if (PR==0 || PR==1) {TString fitRestxt = Form("../roots/EventSelection/SimFitResult_%s_%s_ctau_%.2f.txt",bCont.Data(),kineLabel.Data(),ctauCut);}
  ...
  */
  ...
  //  else if (PR==0 || PR==1) {TFile *wf = new TFile(Form("../roots/EventSelection/SimFitResult_%s_%s_ctau_%.2f_HFUp.root",bCont.Data(), kineLabel.Data(), ctauCut),"recreate"); wf->cd();}
