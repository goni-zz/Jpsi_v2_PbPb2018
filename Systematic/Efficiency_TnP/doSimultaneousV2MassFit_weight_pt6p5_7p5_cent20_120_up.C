...
void doSimultaneousV2MassFit_weight_pt6p5_7p5_cent20_120_up(
    ...
    )
{
  ...
  else if(ctauCut==0){alpha_ = 2.5; n_ = 1.2;}
  ...
  if(ctauCut==-1){c_ = 0.07; c1_ = -0.01428; c2_ = 0.0284097;}
  else if(ctauCut==0){c_ = 0.05; c1_ = -0.01428; c2_ = 0.0284097;}
  else if(ctauCut==1){c_ = 0.07; c1_ = -0.01428; c2_ = 0.0284097;}

  Double_t par0[nParmV];

  gSystem->mkdir(Form ("figs/Efficiency_TnP_up/%s",DATE.Data()), kTRUE);
  gSystem->mkdir(Form("roots/Efficiency_TnP_up/%s",DATE.Data()), kTRUE);
  ...
  /*
     if (PR==2) {TString fitRestxt = Form("../roots/Efficiency_TnP_up/SimFitResult_%s_%s.txt",bCont.Data(),kineLabel.Data());}
     else if (PR==0 || PR==1) {TString fitRestxt = Form("../roots/Efficiency_TnP_up/SimFitResult_%s_%s_ctau_%.2f.txt",bCont.Data(),kineLabel.Data(),ctauCut);}
  ...
  pad2->Draw();
  //c_mass_v2->Update();
  if (ctauCut==0) c_mass_v2->SaveAs(Form("figs/Efficiency_TnP_up/%s/v2Mass_%s_ctau_%.5f_%.5f_%s.pdf",DATE.Data(),kineLabel.Data(),ctauLow,ctauHigh,cutName.Data()));
  else if(ctauCut==-1)c_mass_v2->SaveAs(Form("figs/Efficiency_TnP_up/%s/v2Mass_%s_ctau_%.5f_%s.pdf",DATE.Data(),kineLabel.Data(),ctauLow,cutName.Data()));
  else if(ctauCut==1)c_mass_v2->SaveAs(Form("figs/Efficiency_TnP_up/%s/v2Mass_%s_ctau_%.5f_%s.pdf",DATE.Data(),kineLabel.Data(),ctauHigh,cutName.Data()));
  else if(ctauCut==2)c_mass_v2->SaveAs(Form("figs/Efficiency_TnP_up/%s/v2Mass_%s_Inc.pdf",DATE.Data(),kineLabel.Data()));
  //if (PR==0||PR==1) {c_mass_v2->SaveAs(Form("../figs/Efficiency_TnP_up/v2Mass_%s_%s_ctau_%.2f.pdf",bCont.Data(), kineLabel.Data(),ctauCut));}
  /*
     Double_t xmass[200];
  ...
  */
  TFile *wf;
  if (ctauCut==0) {wf = new TFile(Form("roots/Efficiency_TnP_up/%s/SimFitResult_%s_Eff%d_Acc%d_PtW%d_TnP%d_ctau_%.5f_%.5f_%s.root", DATE.Data(),kineLabel.Data(),fEffW,fAccW,isPtW,isTnP,ctauLow,ctauHigh,cutName.Data()),"recreate"); wf->cd();}
  else if(ctauCut==-1) {wf = new TFile(Form("roots/Efficiency_TnP_up/%s/SimFitResult_%s_Eff%d_Acc%d_PtW%d_TnP%d_ctau_%.5f_%s.root", DATE.Data(), kineLabel.Data(),fEffW,fAccW,isPtW,isTnP,ctauLow,cutName.Data()),"recreate"); wf->cd();}
  else if(ctauCut==1) {wf = new TFile(Form("roots/Efficiency_TnP_up/%s/SimFitResult_%s_Eff%d_Acc%d_PtW%d_TnP%d_ctau_%.5f_%s.root", DATE.Data(), kineLabel.Data(),fEffW,fAccW,isPtW,isTnP,ctauHigh,cutName.Data()),"recreate"); wf->cd();}
  //  else if (PR==0 || PR==1) {TFile *wf = new TFile(Form("../roots/Efficiency_TnP_up/SimFitResult_%s_%s_ctau_%.2f.root",bCont.Data(), kineLabel.Data(), ctauCut),"recreate"); wf->cd();}
