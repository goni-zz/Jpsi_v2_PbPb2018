...
void doSimultaneousV2MassFit_weight_pt12_50_cent40_80_Up(
    ...
    )
{
  ...
  fFinal = new TFile(Form("../../Macros/2021_04_22/roots/2DFit_%s/CtauRes_Sys/2DFitResult_%s_%sw_Effw%d_Accw%d_PtW%d_TnP%d.root", DATE.Data(), kineLabel.Data(), "PR", fEffW, fAccW, isPtW, isTnP));
  ...
  Double_t N1_ = 4500000; Double_t Nbkg_ = 4000000;//Par0, 1
  ...
  else if(ctauCut==0){alpha_ = 2.1; n_ = 1.3;}
  else if(ctauCut==1){alpha_ = 2.5; n_ = 1.5;}
  Double_t ratio_ = 0.35;//par6
  Double_t frac_ = 0.45;//par7
  //if(ctauCut==-1){Bkgp0_=0.1;   Bkgp1_=0.1;   Bkgp2_=0.1;}//par8, 9, 10
  if(ctauCut==-1){Bkgp0_=-0.054908;   Bkgp1_=-0.000670835;   Bkgp2_=0.00622086;}
  else if(ctauCut==0){Bkgp0_=-0.054908;   Bkgp1_=-0.000670835;   Bkgp2_=0.00622086;}
  else if(ctauCut==1){Bkgp0_=-0.054908;   Bkgp1_=-0.000670835;   Bkgp2_=0.00622086;}
  ...
  par0[4] = ws->var("alpha_1_A")->getVal();
  par0[5] = ws->var("n_1_A")->getVal();
  ...
  Double_t parLimitLow[nParmV]  = {par0[0]*0, par0[1]*0, mean_-0.1,  0.01,
    1.01, 0.4, 0,  0,  
    -30, -30, -30, 
    0., -30, -30};
  //Double_t parLimitHigh[nParmV] = {par0[0]*0.4, par0[1], mean_+0.1,  0.9,//Right
  Double_t parLimitHigh[nParmV] = {par0[0]*0.35, par0[1], mean_+0.1,  0.9,//Center
  //Double_t parLimitHigh[nParmV] = {par0[0]*0.4, par0[1], mean_+0.1,  0.9,//Right
  ...
  else if(ctauCut==-1)c_mass_v2->SaveAs(Form("figs/v2mass_fit/%s/v2Mass_%s_ctau_%.5f_%s.pdf",DATE.Data(),kineLabel.Data(),ctauLow,cutName.Data()));
  else if(ctauCut==1)c_mass_v2->SaveAs(Form("figs/v2mass_fit/%s/v2Mass_%s_ctau_%.5f_%s.pdf",DATE.Data(),kineLabel.Data(),ctauHigh,cutName.Data()));
  else if(ctauCut==2)c_mass_v2->SaveAs(Form("figs/v2mass_fit/%s/v2Mass_%s_Inc.pdf",DATE.Data(),kineLabel.Data()));
