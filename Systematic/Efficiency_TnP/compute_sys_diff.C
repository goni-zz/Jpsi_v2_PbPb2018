...
void compute_sys_diff()
{
  ...
  TString dirAlt = "/Users/goni/Documents/CMS/Analysis/Jpsi_Flow/Usercode/Jpsi_v2_PbPb2018/Systematic/Efficiency_TnP/";
  ...
  for(int ib=0; ib<nPtBinPR; ib++){
    ...
    if(ib<2) fileAltPR = dirAlt+Form("Efficiency_TnP_v2_pt%.1f-%.1f_y1.6-2.4_muPt0.0_centrality20-120.root",PtBinArrPR[ib],PtBinArrPR[ib+1]);
    else fileAltPR = dirAlt+Form("Efficiency_TnP_v2_pt%.1f-%.1f_y0.0-2.4_muPt0.0_centrality20-120.root",PtBinArrPR[ib],PtBinArrPR[ib+1]);
  ...
  for(int ib=0; ib<nPtBinNP; ib++){
    ...
    if(ib<1) fileAltNP = dirAlt+Form("Efficiency_TnP_v2_pt%.1f-%.1f_y1.6-2.4_muPt0.0_centrality20-120.root",PtBinArrNP[ib],PtBinArrNP[ib+1]);
    else fileAltNP = dirAlt+Form("Efficiency_TnP_v2_pt%.1f-%.1f_y0.0-2.4_muPt0.0_centrality20-120.root",PtBinArrNP[ib],PtBinArrNP[ib+1]);
  ...
  TFile *wf = new TFile("Efficiency_TnP_sys.root","recreate");
  wf->cd();
  hPtSys_PR->Write();
  hPtSys_NP->Write();
  //for(int ib=0; ib<nPtBinPR; ib++){
...
{ 
  ...
  cout << "PR Unc : " << Form("%.3f",100*double(sys)) << "%%" << endl;
  return sys;
}
...
{ 
  ...
  sys = fabs(v2_alt-v2_nom);
  cout << "NP Unc : " << Form("%.3f",100*double(sys)) << "%%" << endl;
  return sys;
}
