...
void doSimultaneousV2MassFit_weight_pt3_6p5_cent20_120_Up(
    int ctauCut=-1,
    ...
    )
{
  ...
  TFile *rf;
  if(ctauCut==0) rf = new TFile(Form("../../roots/TnPSysUp_210928/v2mass_hist/%s/Jpsi_%s_Eff%d_Acc%d_PtW%d_TnP%d_ctau_%.5f_%.5f_%s.root",
        DATE.Data(),kineLabelTot.Data(),fEffW,fAccW,isPtW,isTnP,ctauLow,ctauHigh,cutName.Data()),"read");
  else if(ctauCut==-1) rf = new TFile(Form("../../roots/TnPSysUp_210928/v2mass_hist/%s/Jpsi_%s_Eff%d_Acc%d_PtW%d_TnP%d_ctau_%.5f_%s.root",
  ...
  else if(ctauCut==2) rf = new TFile(Form("../../roots/TnPSysUp_210928/v2mass_hist/%s/Jpsi_%s_Eff%d_Acc%d_PtW%d_TnP%d_Inc.root",
  ...
  double n_low  = wsmc->var("n_1_A")->getVal()-wsmc->var("n_1_A")->getError()*5;
  double n_high = wsmc->var("n_1_A")->getVal()+wsmc->var("n_1_A")->getError()*5;
