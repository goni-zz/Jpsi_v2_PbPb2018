...
void mergeSys_TnP(int nbin=1) { 
  ...
  TFile* f1 = new TFile(Form("TnPUp_sys_%s.root",bin.Data()));
  h_Pt1_PR[1] = (TH1D*) f1->Get("hPtSys_PR");
  h_Pt1_NP[1] = (TH1D*) f1->Get("hPtSys_NP");
  
  // 2 : Event Selection Down
  TFile* f2 = new TFile(Form("TnPDw_sys_%s.root",bin.Data()));
  ...
  TFile* fout = new TFile(Form("TnP_sys_%s.root",bin.Data()),"recreate");
  fout->cd();
  h_Pt1_PR[0]->Write();
  h_Pt1_NP[0]->Write();
