...
void doSimultaneousV2MassFit_weight_pt3_6p5_cent20_120(
    float ptLow =  6.5, float ptHigh = 50,
    float yLow = 0, float yHigh = 2.4,
    int cLow = 20, int cHigh = 120,
    int ctauCut=-1,
    int weight_PR = 0, //PR : 0, NP : 1
    bool fEffW=true, bool fAccW=true, bool isPtW=true, bool isTnP=false,
    ...
    )
{
  ...
  fFinal = new TFile(Form("../../Macros/2021_04_22/roots/2DFit_%s/Final/2DFitResult_%s_%sw_Effw%d_Accw%d_PtW%d_TnP%d_v2.root", DATE.Data(), kineLabel.Data(), "PR", fEffW, fAccW, 1, 1));
  ...
  gSystem->mkdir(Form ("figs/Efficiency_TnP/%s",DATE.Data()), kTRUE);
  gSystem->mkdir(Form("roots/Efficiency_TnP/%s",DATE.Data()), kTRUE);
  ...
  /*
     if (PR==2) {TString fitRestxt = Form("../roots/Efficiency_TnP/SimFitResult_%s_%s.txt",bCont.Data(),kineLabel.Data());}
     else if (PR==0 || PR==1) {TString fitRestxt = Form("../roots/Efficiency_TnP/SimFitResult_%s_%s_ctau_%.2f.txt",bCont.Data(),kineLabel.Data(),ctauCut);}
  ...
  //drawText(Form("#alpha(#frac{{S}{S+B}})= %.3f",fAlpha->GetMaximum()),pos_x_mass+0.45,pos_y+pos_y_diff*1,text_color,text_size+2);
  drawText(Form("#alpha(S/S+B)= %.3f",fAlpha->GetMaximum()),pos_x_mass+0.45,pos_y+pos_y_diff*1,text_color,text_size+2);
  ...
  pad2->Draw();
  //c_mass_v2->Update();
  if (ctauCut==0) c_mass_v2->SaveAs(Form("figs/Efficiency_TnP/%s/v2Mass_%s_ctau_%.5f_%.5f_%s.pdf",DATE.Data(),kineLabel.Data(),ctauLow,ctauHigh,cutName.Data()));
  else if(ctauCut==-1)c_mass_v2->SaveAs(Form("figs/Efficiency_TnP/%s/v2Mass_%s_ctau_%.5f_%s.pdf",DATE.Data(),kineLabel.Data(),ctauLow,cutName.Data()));
  else if(ctauCut==1)c_mass_v2->SaveAs(Form("figs/Efficiency_TnP/%s/v2Mass_%s_ctau_%.5f_%s.pdf",DATE.Data(),kineLabel.Data(),ctauHigh,cutName.Data()));
  else if(ctauCut==2)c_mass_v2->SaveAs(Form("figs/Efficiency_TnP/%s/v2Mass_%s_Inc.pdf",DATE.Data(),kineLabel.Data()));
  //if (PR==0||PR==1) {c_mass_v2->SaveAs(Form("../figs/Efficiency_TnP/v2Mass_%s_%s_ctau_%.2f.pdf",bCont.Data(), kineLabel.Data(),ctauCut));}
  /*
     Double_t xmass[200];
  ...
  */
  TFile *wf;
  if (ctauCut==0) {wf = new TFile(Form("roots/Efficiency_TnP/%s/SimFitResult_%s_Eff%d_Acc%d_PtW%d_TnP%d_ctau_%.5f_%.5f_%s.root", DATE.Data(),kineLabel.Data(),fEffW,fAccW,isPtW,isTnP,ctauLow,ctauHigh,cutName.Data()),"recreate"); wf->cd();}
  else if(ctauCut==-1) {wf = new TFile(Form("roots/Efficiency_TnP/%s/SimFitResult_%s_Eff%d_Acc%d_PtW%d_TnP%d_ctau_%.5f_%s.root", DATE.Data(), kineLabel.Data(),fEffW,fAccW,isPtW,isTnP,ctauLow,cutName.Data()),"recreate"); wf->cd();}
  else if(ctauCut==1) {wf = new TFile(Form("roots/Efficiency_TnP/%s/SimFitResult_%s_Eff%d_Acc%d_PtW%d_TnP%d_ctau_%.5f_%s.root", DATE.Data(), kineLabel.Data(),fEffW,fAccW,isPtW,isTnP,ctauHigh,cutName.Data()),"recreate"); wf->cd();}
  //  else if (PR==0 || PR==1) {TFile *wf = new TFile(Form("../roots/Efficiency_TnP/SimFitResult_%s_%s_ctau_%.2f.root",bCont.Data(), kineLabel.Data(), ctauCut),"recreate"); wf->cd();}
