...
void doSimultaneousV2MassFit_weight_pt3_4p5_cent20_120_free(
    int ctauCut=-1,
    ...
    )
{
  ...
  if(ctauCut==-1){c_ = 0.076; c1_ = -0.01428; c2_ = 0.0284097;}
  else if(ctauCut==0){c_ = 0.112; c1_ = -0.01428; c2_ = 0.0284097;}
  else if(ctauCut==1){c_ = 0.80; c1_ = -0.01428; c2_ = 0.0284097;}
  ...
  for(int ipar = 0; ipar<nParmV_; ipar++){
    ...
    if(ipar==6) fitter.Config().ParSettings(ipar).Fix();
    else fitter.Config().ParSettings(ipar).SetLimits(parLimitLow[ipar],parLimitHigh[ipar]);
    //fitter.Config().ParSettings(ipar).SetLimits(parLimitLow[ipar],parLimitHigh[ipar]);
  }
  ...
  pad2->Draw();
  //c_mass_v2->Update();
  if (ctauCut==0) c_mass_v2->SaveAs(Form("figs/SignalPDFvariation/%s/v2Mass_%s_ctau_%.5f_%.5f_%s_free.pdf",DATE.Data(),kineLabel.Data(),ctauLow,ctauHigh,cutName.Data()));
  else if(ctauCut==-1)c_mass_v2->SaveAs(Form("figs/SignalPDFvariation/%s/v2Mass_%s_ctau_%.5f_%s_free.pdf",DATE.Data(),kineLabel.Data(),ctauLow,cutName.Data()));
  else if(ctauCut==1)c_mass_v2->SaveAs(Form("figs/SignalPDFvariation/%s/v2Mass_%s_ctau_%.5f_%s_free.pdf",DATE.Data(),kineLabel.Data(),ctauHigh,cutName.Data()));
  ...
  */
  TFile *wf;
  if (ctauCut==0) {wf = new TFile(Form("roots/SignalPDFvariation/%s/SimFitResult_%s_Eff%d_Acc%d_PtW%d_TnP%d_ctau_%.5f_%.5f_%s_free.root", DATE.Data(),kineLabel.Data(),fEffW,fAccW,isPtW,isTnP,ctauLow,ctauHigh,cutName.Data()),"recreate"); wf->cd();}
  else if(ctauCut==-1) {wf = new TFile(Form("roots/SignalPDFvariation/%s/SimFitResult_%s_Eff%d_Acc%d_PtW%d_TnP%d_ctau_%.5f_%s_free.root", DATE.Data(), kineLabel.Data(),fEffW,fAccW,isPtW,isTnP,ctauLow,cutName.Data()),"recreate"); wf->cd();}
  else if(ctauCut==1) {wf = new TFile(Form("roots/SignalPDFvariation/%s/SimFitResult_%s_Eff%d_Acc%d_PtW%d_TnP%d_ctau_%.5f_%s_free.root", DATE.Data(), kineLabel.Data(),fEffW,fAccW,isPtW,isTnP,ctauHigh,cutName.Data()),"recreate"); wf->cd();}
