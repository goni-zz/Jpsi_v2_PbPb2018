...
void vnlinearfit_Cent(
  ...
  else if(ptLow==6.5&&ptHigh==50) SimFit = "0_180";
  
  TFile *fFinal; 
  TFile *fv2fit1;
  TFile *fv2fit2;
  TFile *fv2fit3;
  fFinal = new TFile(Form("../../Macros/2021_09_14/roots/2DFit_%s/CtauRes_Sys/2DFitResult_%s_%sw_Effw%d_Accw%d_PtW%d_TnP%d.root", SimFit.Data(), kineLabel.Data(), "PR", fEffW, 1, isPtW, isTnP));
  //if(ptLow==3&&ptHigh==6.5)fFinal = new TFile(Form("../../Macros/2021_09_14/roots/2DFit_%s/CtauRes_Sys/2DFitResult_%s_%sw_Effw%d_Accw%d_PtW%d_TnP%d_v2.root", SimFit.Data(), kineLabel.Data(), "PR", fEffW, 1, isPtW, isTnP));
  //else fFinal = new TFile(Form("../../Macros/2021_09_14/roots/2DFit_%s/CtauRes_Sys/2DFitResult_%s_%sw_Effw%d_Accw%d_PtW%d_TnP%d.root", SimFit.Data(), kineLabel.Data(), "PR", fEffW, 1, isPtW, isTnP));
