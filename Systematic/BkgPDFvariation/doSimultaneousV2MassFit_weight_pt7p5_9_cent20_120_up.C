...
Double_t TotalYieldBkg(Double_t* x, Double_t* par)
{
  ...
  Double_t BkgM = Nbkg*(TMath::Exp(cheb1+x[0]*cheb0));
...
Double_t Totalvnpol1JPsi(Double_t* x, Double_t* par)
{
  ...
  Double_t BkgM = Nbkg*(TMath::Exp(Bkgp1+x[0]*Bkgp0));
...
Double_t Totalvnpol3JPsi(Double_t* x, Double_t* par)
{
  ...
  Double_t frac = par[9];
  Double_t Bkgp0 = par[10];
  Double_t Bkgp1 = par[11];
  Double_t Bkgp2 = par[12];
  Double_t c = par[13];
  Double_t c1 = par[14];
  Double_t c2 = par[15];
  Double_t c3 = par[16];
...
Double_t vnPol1BkgAlpha(Double_t* x, Double_t* par)
{
  ...
  Double_t SigM = N1*(frac*JPsi_1 + fN_2*(1-frac)*JPsi_2);
...
void doSimultaneousV2MassFit_weight_pt7p5_9_cent20_120_up(
    ...
    )
{
  ...
  gSystem->mkdir(Form ("figs/BkgPDFvariation_up/%s",DATE.Data()), kTRUE);
  gSystem->mkdir(Form("roots/BkgPDFvariation_up/%s",DATE.Data()), kTRUE);
  ...
  /*
     if (PR==2) {TString fitRestxt = Form("../roots/BkgPDFvariation_up/SimFitResult_%s_%s.txt",bCont.Data(),kineLabel.Data());}
     else if (PR==0 || PR==1) {TString fitRestxt = Form("../roots/BkgPDFvariation_up/SimFitResult_%s_%s_ctau_%.2f.txt",bCont.Data(),kineLabel.Data(),ctauCut);}
  ...
  pad2->Draw();
  //c_mass_v2->Update();
  if (ctauCut==0) c_mass_v2->SaveAs(Form("figs/BkgPDFvariation_up/%s/v2Mass_%s_ctau_%.5f_%.5f_%s.pdf",DATE.Data(),kineLabel.Data(),ctauLow,ctauHigh,cutName.Data()));
  else if(ctauCut==-1)c_mass_v2->SaveAs(Form("figs/BkgPDFvariation_up/%s/v2Mass_%s_ctau_%.5f_%s.pdf",DATE.Data(),kineLabel.Data(),ctauLow,cutName.Data()));
  else if(ctauCut==1)c_mass_v2->SaveAs(Form("figs/BkgPDFvariation_up/%s/v2Mass_%s_ctau_%.5f_%s.pdf",DATE.Data(),kineLabel.Data(),ctauHigh,cutName.Data()));
  else if(ctauCut==2)c_mass_v2->SaveAs(Form("figs/BkgPDFvariation_up/%s/v2Mass_%s_Inc.pdf",DATE.Data(),kineLabel.Data()));
  //if (PR==0||PR==1) {c_mass_v2->SaveAs(Form("../figs/BkgPDFvariation_up/v2Mass_%s_%s_ctau_%.2f.pdf",bCont.Data(), kineLabel.Data(),ctauCut));}
  /*
     Double_t xmass[200];
  ...
  */
  TFile *wf;
  if (ctauCut==0) {wf = new TFile(Form("roots/BkgPDFvariation_up/%s/SimFitResult_%s_Eff%d_Acc%d_PtW%d_TnP%d_ctau_%.5f_%.5f_%s.root", DATE.Data(),kineLabel.Data(),fEffW,fAccW,isPtW,isTnP,ctauLow,ctauHigh,cutName.Data()),"recreate"); wf->cd();}
  else if(ctauCut==-1) {wf = new TFile(Form("roots/BkgPDFvariation_up/%s/SimFitResult_%s_Eff%d_Acc%d_PtW%d_TnP%d_ctau_%.5f_%s.root", DATE.Data(), kineLabel.Data(),fEffW,fAccW,isPtW,isTnP,ctauLow,cutName.Data()),"recreate"); wf->cd();}
  else if(ctauCut==1) {wf = new TFile(Form("roots/BkgPDFvariation_up/%s/SimFitResult_%s_Eff%d_Acc%d_PtW%d_TnP%d_ctau_%.5f_%s.root", DATE.Data(), kineLabel.Data(),fEffW,fAccW,isPtW,isTnP,ctauHigh,cutName.Data()),"recreate"); wf->cd();}
  //  else if (PR==0 || PR==1) {TFile *wf = new TFile(Form("../roots/BkgPDFvariation_up/SimFitResult_%s_%s_ctau_%.2f.root",bCont.Data(), kineLabel.Data(), ctauCut),"recreate"); wf->cd();}
