...
void vnlinearfit_Pt(
  ...
  TFile *fv2fit2;
  TFile *fv2fit3;
  fFinal = new TFile(Form("../../Macros/2021_09_14/roots/2DFit_%s/Final/2DFitResult_%s_%sw_Effw%d_Accw%d_PtW%d_TnP%d.root", SimFit.Data(), kineLabel.Data(), "PR", fEffW, fAccW, 1, isTnP));
  //if(ptLow==3&&ptHigh==6.5)fFinal = new TFile(Form("../../Macros/2021_09_14/roots/2DFit_%s/Final/2DFitResult_%s_%sw_Effw%d_Accw%d_PtW%d_TnP%d_v2.root", SimFit.Data(), kineLabel.Data(), "PR", fEffW, fAccW, 1, isTnP));
  //else fFinal = new TFile(Form("../../Macros/2021_09_14/roots/2DFit_%s/Final/2DFitResult_%s_%sw_Effw%d_Accw%d_PtW%d_TnP%d.root", SimFit.Data(), kineLabel.Data(), "PR", fEffW, fAccW, 1, isTnP));
  ...
  fv2fit1=new TFile(Form("roots/Efficiency_PtW/%s/SimFitResult_%s_Eff%d_Acc%d_PtW%d_TnP%d_ctau_%.5f_%s.root", SimFit.Data(),kineLabel.Data(),fEffW,fAccW,isPtW,isTnP,ctauLow,"ctauL"));
  fv2fit2=new TFile(Form("roots/Efficiency_PtW/%s/SimFitResult_%s_Eff%d_Acc%d_PtW%d_TnP%d_ctau_%.5f_%.5f_%s.root", SimFit.Data(),kineLabel.Data(),fEffW,fAccW,isPtW,isTnP,ctauLow,ctauHigh,"ctauC"));
  fv2fit3=new TFile(Form("roots/Efficiency_PtW/%s/SimFitResult_%s_Eff%d_Acc%d_PtW%d_TnP%d_ctau_%.5f_%s.root", SimFit.Data(),kineLabel.Data(),fEffW,fAccW,isPtW,isTnP,ctauHigh,"ctauR"));
  ...
  c1->SaveAs(Form("Efficiency_PtW_fraction_vs_v2_%s.pdf",kineLabel.Data()));
  ...
  TFile *fv2 = new TFile(Form("Efficiency_PtW_v2_%s.root",kineLabel.Data()),"RECREATE");
  fv2->cd();
  prv2->Write();
  npv2->Write();
}
