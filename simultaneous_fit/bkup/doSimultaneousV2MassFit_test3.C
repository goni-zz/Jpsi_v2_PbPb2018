...
Double_t Totalvnpol2JPsi(Double_t* x, Double_t* par)
{
  ...
  Double_t SigM1s = N1*(frac*JPsi_1 + (1-frac)*JPsi_2);
  Double_t SigM = N1*(JPsi_1);
  ...
  return c*(SigM1s/(SigM1s+BkgM)) + (1 - SigM/(SigM+BkgM))*(c3 + c2*x[0] + c1*x[0]*x[0]);
...
void fitting_curve(
    TF1* fmass_total,
    TF1* fvn_simul,
    TGraphAsymmErrors* g_mass,
    TH1D* h_v2_SplusB,
    Double_t parLimitLow[nParmV],
    Double_t parLimitHigh[nParmV],
    Double_t N1_ , Double_t Nbkg_, Double_t mean_, Double_t sigma_, 
    Double_t alpha_, Double_t n_, Double_t ratio_, Double_t frac_, Double_t Bkgp0_, 
    Double_t c_, Double_t c1_, Double_t c2_, Double_t c3_, int nParmV_, int nParBkg,
    float cLow, float cHigh,
    float ptLow, float ptHigh,
    float yLow, float yHigh,
    float SiMuPtCut, float massLow, float massHigh,
    TString CtauBin=""
    );

void doSimultaneousV2MassFit_test3(int cLow = 20, int cHigh = 120,
    ...
    float SiMuPtCut = 0, float massLow = 2.6, float massHigh =3.5, bool dimusign=true, int ibkg_vn_sel = fpol2)
{
  ...
  int nCtauBin = 9;
  //TFile* rf = new TFile(Form("../roots/v2mass_hist/Jpsi_Inclusive_%s.root",kineLabel.Data()),"read");
  TFile* rf = new TFile("../roots/ctau_mass/step1_pt0.0-50.0_y0.0-2.4_muPt0.0_centrality20-120.root","read");
  TGraphAsymmErrors* g_mass[nCtauBin];
  for(int i=0; i<nCtauBin; i++){
  g_mass[i] = (TGraphAsymmErrors*) rf->Get(Form("g_mass_%d", i));  
  }
  TH1D* h_v2_SplusB = (TH1D*) rf->Get("h_v2_SplusB");  

  TFile *wf = new TFile(Form("../roots/v2mass_fit/SimFitResult_Inclusive_%s.root", kineLabel.Data()),"recreate");

  TF1* fmass_total = new TF1("fmass_total", TotalYield, massLow, massHigh, nParmM);
  TF1* fvn_simul = new TF1("fvn_simul", Totalvnpol2JPsi, massLow, massHigh, nParmV_);
  ...
  //                                N1_ ,   Nbkg_,       mean_,sigma_,alpha_,    n_, ratio_, frac_, Bkgp0_, c_, c1_, c2_, c3_
  Double_t parLimitLow[nParmV]  = {      0,     0, mean_ -0.02,  0.04,     1,     1,   0, 0,      0, -0.5, -5, -5, -4};
  Double_t parLimitHigh[nParmV] = {N1_*5, Nbkg_*5, mean_ +0.02,  0.06,     3,     3,   1, 1,     25,  0.5,  5,  5,  4};

  fitting_curve(
    fmass_total, fvn_simul, g_mass[3], h_v2_SplusB, 
    parLimitLow, parLimitHigh, 
    N1_, Nbkg_, mean_ = pdgMass.JPsi, sigma_ = sigma_,
    alpha_, n_, ratio_, frac_, Bkgp0_, c_, c1_, c2_, c3_, nParmV_ =15, nParBkg =3,
    cLow, cHigh, ptLow, ptHigh, yLow, yHigh, SiMuPtCut = 0, massLow, massHigh,
    "3"
    );
  //fitting_curve(
  //  fmass_total, fvn_simul, g_mass[2], h_v2_SplusB, 
  //  parLimitLow, parLimitHigh, 
  //  N1_, Nbkg_, mean_ = pdgMass.JPsi, sigma_ = sigma_,
  //  alpha_, n_, ratio_, frac_, Bkgp0_, c_, c1_, c2_, c3_, nParmV_ =15, nParBkg =3,
  //  cLow, cHigh, ptLow, ptHigh, yLow, yHigh, SiMuPtCut = 0, massLow, massHigh
  ...
  //  );
  //fitting_curve(
  //  fmass_total, fvn_simul, g_mass[4], h_v2_SplusB, 
  //  parLimitLow, parLimitHigh, 
  //  N1_, Nbkg_, mean_ = pdgMass.JPsi, sigma_ = sigma_,
  //  alpha_, n_, ratio_, frac_, Bkgp0_, c_, c1_, c2_, c3_, nParmV_ =15, nParBkg =3,
  //  cLow, cHigh, ptLow, ptHigh, yLow, yHigh, SiMuPtCut = 0, massLow, massHigh
  //  );
  //fitting_curve(
  //  fmass_total, fvn_simul, g_mass[5], h_v2_SplusB, 
  //  parLimitLow, parLimitHigh, 
  //  N1_, Nbkg_, mean_ = pdgMass.JPsi, sigma_ = sigma_,
  //  alpha_, n_, ratio_, frac_, Bkgp0_, c_, c1_, c2_, c3_, nParmV_ =15, nParBkg =3,
  //  cLow, cHigh, ptLow, ptHigh, yLow, yHigh, SiMuPtCut = 0, massLow, massHigh
  //  ); 
  //fitting_curve(
  //  fmass_total, fvn_simul, g_mass[6], h_v2_SplusB, 
  //  parLimitLow, parLimitHigh, 
  //  N1_, Nbkg_, mean_ = pdgMass.JPsi, sigma_ = sigma_,
  //  alpha_, n_, ratio_, frac_, Bkgp0_, c_, c1_, c2_, c3_, nParmV_ =15, nParBkg =3,
  //  cLow, cHigh, ptLow, ptHigh, yLow, yHigh, SiMuPtCut = 0, massLow, massHigh
  //  );
  //fitting_curve(
  //  fmass_total, fvn_simul, g_mass[7], h_v2_SplusB, 
  //  parLimitLow, parLimitHigh, 
  //  N1_, Nbkg_, mean_ = pdgMass.JPsi, sigma_ = sigma_,
  //  alpha_, n_, ratio_, frac_, Bkgp0_, c_, c1_, c2_, c3_, nParmV_ =15, nParBkg =3,
  //  cLow, cHigh, ptLow, ptHigh, yLow, yHigh, SiMuPtCut = 0, massLow, massHigh
...
//  Double_t N1_ = 2300.00;
//  Double_t Nbkg_ = 2300.00;
//  Double_t mean_ = pdgMass.JPsi;
//  Double_t sigma_ = 1.1827e-02;
//  //Double_t alpha_ = 1.526;
//  Double_t alpha_ = 1.526;
...
//  //par0[13] = c4_;
//
//  Double_t parLimitLow[nParmV]  = {      0,     0, mean_ -0.02,  0.04,     1,     1,   0, 0,      -25,    0, -5, -5, -4};
//  Double_t parLimitHigh[nParmV] = {N1_*5, Nbkg_*5, mean_ +0.02,  0.06,     3,     3,   1, 1,     50,  0.5,  5,  5,  4};
...
//  drawText(Form("NJpsi: %.2f #pm %.1f",fvn_simul->GetParameter(0), fvn_simul->GetParError(0)),pos_x_mass,pos_y-pos_y_diff*5,text_color,text_size);
//  //drawText("L_{J/#psi} 0.0625",pos_x_mass,pos_y-pos_y_diff*5,text_color,text_size);
//
//  pad1->Update();
//
//  c_mass_v2->cd();
...
void fitting_curve(
    ...
    Double_t parLimitLow[nParmV],
    Double_t parLimitHigh[nParmV],
    Double_t N1_ = 2300.00, Double_t Nbkg_ = 2300.00, Double_t mean_ = pdgMass.JPsi, Double_t sigma_ = 1.1827e-02, 
    Double_t alpha_ = 1.526, Double_t n_ = 2.1, Double_t ratio_ = 0.3750, Double_t frac_ = 0.5, Double_t Bkgp0_ = -3.22817, 
    Double_t c_ = 0.044, Double_t c1_ = 0.00342884, Double_t c2_ = -0.0684097, Double_t c3_ = 0.04, int nParmV_=15, int nParBkg=3,
  ...
  fvn_simul = new TF1("fvn_simul", Totalvnpol2JPsi, massLow, massHigh, nParmV_);
  fvn_simul->SetLineColor(2);
  fvn_simul->SetLineWidth(1);
  ...
  drawText(Form("p_{T}^{#mu} > %.1f GeV/c", SiMuPtCut ), pos_x_mass,pos_y-pos_y_diff*2,text_color,text_size);
  drawText("|#eta^{#mu}| < 2.4", pos_x_mass,pos_y-pos_y_diff*3,text_color,text_size);
  drawText(Form("Centrality %.0f-%.0f%s",cLow/2,cHigh/2,perc.Data()),pos_x_mass,pos_y-pos_y_diff*4,text_color,text_size);
  drawText(Form("NJpsi: %.2f #pm %.1f",fvn_simul->GetParameter(0), fvn_simul->GetParError(0)),pos_x_mass,pos_y-pos_y_diff*5,text_color,text_size);
  //drawText("L_{J/#psi} 0.0625",pos_x_mass,pos_y-pos_y_diff*5,text_color,text_size);
  ...
  pad2->Draw();
  c_mass_v2->Update();
  c_mass_v2->SaveAs(Form("../figs/v2mass_fit/v2Mass_Inclusive_%s_%s.pdf", kineLabel.Data(), CtauBin.Data()));

  TFile *wf = new TFile(Form("../roots/v2mass_fit/SimFitResult_Inclusive_%s_%s.root", kineLabel.Data(), CtauBin.Data()),"recreate");
  wf->cd();
  //store individual function{{{
  fvn_simul->Write();
  fyieldtot = (TF1*) fmass_total->Clone();
