...
void doSimultaneousV2MassFit_MC_test1(int cLow = 20, int cHigh = 120,
    ...
    float SiMuPtCut = 0, float massLow = 2.6, float massHigh =3.5, bool dimusign=true, int ibkg_vn_sel = fpol2)
{
  ...
  TFile* rf = new TFile("../roots/ctau_mass/step1_pt0.0-50.0_y0.0-2.4_muPt0.0_centrality20-120_MC_PR.root","read");
  ...
  TFile *wf = new TFile(Form("../roots/v2mass_fit/SimFitResult_Inclusive__MC_PR_%s.root", kineLabel.Data()),"recreate");
...
void fitting_curve(
  ...
  pad2->Draw();
  c_mass_v2->Update();
  c_mass_v2->SaveAs(Form("../figs/v2mass_fit/v2Mass_Inclusive_%s_MC_PR_%s.pdf", kineLabel.Data(), CtauBin.Data()));

  TFile *wf = new TFile(Form("../roots/v2mass_fit/SimFitResult_Inclusive_%s_MC_PR_%s.root", kineLabel.Data(), CtauBin.Data()),"recreate");
  wf->cd();
  //store individual function{{{
  fvn_simul->Write();
  fyieldtot = (TF1*) fmass_total->Clone();
