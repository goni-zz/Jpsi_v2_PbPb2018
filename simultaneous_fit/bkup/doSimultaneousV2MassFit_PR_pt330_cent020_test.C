...
void doSimultaneousV2MassFit_PR_pt330_cent020_test(int cLow = 0, int cHigh = 20,
    ...
    float SiMuPtCut = 0, float massLow = 2.6, float massHigh =3.5, bool dimusign=true, int ibkg_vn_sel = fpol2)
{
  ...
  TFile* rf = new TFile(Form("../roots/v2mass_hist/Jpsi_Prompt_%s.root",kineLabel.Data()),"read");
  ...
  TFile *wf = new TFile(Form("../roots/v2mass_fit/SimFitResult_Prompt_%s.root", kineLabel.Data()),"recreate");
  ...
  Double_t parLimitHigh[nParmV] = {N1_*5, Nbkg_*5, mean_ +0.02,  0.06,     9,     9,   1, 1,     50,  0.5,  5,  5,  4};
...
//  fyield_bkg->SetFillColor(kAzure-9);
  ...
  drawText("L_{J/#psi} 0.0385",pos_x_mass,pos_y-pos_y_diff*5,text_color,text_size);
