    ...
    float SiMuPtCut = 0, float massLow = 2.6, float massHigh =3.5, bool dimusign=true, int ibkg_vn_sel = fpol2)
{
  ...
  TFile* rf = new TFile(Form("../roots/v2mass_hist/Jpsi_NonPrompt_%s.root",kineLabel.Data()),"read");
  ...
  TFile *wf = new TFile(Form("../roots/v2mass_fit/SimFitResult_NonPrompt_%s.root", kineLabel.Data()),"recreate");
  ...
  Double_t parLimitHigh[nParmV] = {N1_*5, Nbkg_*5, mean_ +0.02,  0.06,     2, 3.2, 1, 1,     25,  0.5,  5,  5,  4};
...
//  fyield_bkg->SetFillColor(kAzure-9);
  ...
  drawText("L_{J/#psi} 0.1155",pos_x_mass,pos_y-pos_y_diff*5,text_color,text_size);
  ...
  pad2->Draw();
  c_mass_v2->Update();
  c_mass_v2->SaveAs(Form("../figs/v2mass_fit/v2Mass_NonPrompt_%s.pdf", kineLabel.Data()));
  /*
     Double_t xmass[200];
