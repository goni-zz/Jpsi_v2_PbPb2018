    ...
    ) //PR 0: PR, 1: NP, 2: Inc.
{
  ...
  gSystem->mkdir(Form("figs/v3mass_hist/%s",DATE.Data()), kTRUE);
  gSystem->mkdir(Form("figs/q_vector_v3/%s",DATE.Data()), kTRUE);
  gSystem->mkdir(Form("figs/mass_dist_v3/%s",DATE.Data()), kTRUE);
  gSystem->mkdir(Form("figs/decayL_v3/%s",DATE.Data()), kTRUE);
  gSystem->mkdir(Form("roots/v3mass_hist/%s",DATE.Data()), kTRUE);
  ...
  fFinal = new TFile(Form("Macros/2021_04_22/roots/2DFit_%s/Final/2DFitResult_%s_%sw_Effw%d_Accw%d_PtW%d_TnP%d.root", DATE.Data(), kineLabel.Data(), "PR", 1, 1, 1, 1));
  ...
  TH1D* h_decayL_v3 = new TH1D("h_decayL_v3",";l_{J/#psi(mm)};Counts/(0.1 mm)",100,-4,6);
  ...
  TLine *lcut;
  //for(int bins=0; bins<h_decayL_v3->GetNbinsX(); bins++){
  //  h_decayL_v3->Integral(0,bins);
  //  if(h_decayL_v3->Integral(0,bins)>=0.79*h_decayL_v3->GetEntries()
  //      &&h_decayL_v3->Integral(0,bins)<=0.81*h_decayL_v3->GetEntries()){ 
  //    cout<<"THE BIN IS: "<<bins<<endl;
  //    lcut = new TLine(h_decayL_v3->GetBinCenter(bins), 0, h_decayL_v3->GetBinCenter(bins), h_decayL_v3->GetMaximum());
  //  }
  //}
  //for(int bins=0; bins<h_decayL_v3->GetNbinsX(); bins++){
  //  h_decayL_v3->Integral(0,bins);
  //  if(h_decayL_v3->Integral(0,bins)<=0.89*h_decayL_v3->GetEntries() || h_decayL_v3->Integral(0,bins)>0.90*h_decayL_v3->GetEntries()) continue;
  //    cout<<"ANSWER: "<<bins<<"th bin"<<", ljpsi: "<<h_decayL_v3->GetBinCenter(bins)<<endl;
  //    lcut = new TLine(h_decayL_v3->GetBinCenter(bins), 0, h_decayL_v3->GetBinCenter(bins), 1);
  //}
  ...
  h_v2_final->GetYaxis()->SetRangeUser(-0.11,0.26);
  ...
  //drawText(Form("NP J/#psi Frac. %.3f", bfrac2),pos_x_mass,pos_y-pos_y_diff*5,text_color,text_size);}
    ...
    pad1->Draw();
    pad2->Draw();
    //gSystem->mkdir(Form("figs/q_vector_v3",sample.Data()),1);
    if(ctauCut==0) c_mass_v2->SaveAs(Form("figs/v3mass_hist/%s/v2Mass_%s_ctau_%.5f_%.5f_%s.pdf",DATE.Data(),kineLabel.Data(),ctauLo,ctauHi,cutName.Data()));
    else if(ctauCut==-1)c_mass_v2->SaveAs(Form("figs/v3mass_hist/%s/v2Mass_%s_ctau_%.5f_%s.pdf",DATE.Data(),kineLabel.Data(),ctauLo,cutName.Data()));
    else if(ctauCut== 1)c_mass_v2->SaveAs(Form("figs/v3mass_hist/%s/v2Mass_%s_ctau_%.5f_%s.pdf",DATE.Data(),kineLabel.Data(),ctauHi,cutName.Data()));
    else c_mass_v2->SaveAs(Form("figs/v3mass_hist/%s/v2Mass_%s_Inc.pdf",DATE.Data(),kineLabel.Data()));

    TCanvas* c_decayL_v3 = new TCanvas("c_decayL_v3","",0,500,600,600);
    c_decayL_v3->SetLogy();
    if(ptLow==0) drawText(Form("p_{T}^{#mu#mu} < %.f GeV/c",ptHigh ),pos_x_mass,pos_y,text_color,text_size);
    ...
    //lcut->Draw("same");
    c_decayL_v3->Update();
    if(ctauCut==0) c_decayL_v3->SaveAs(Form("figs/decayL_v3/%s/decayL_v3_%s_ctau_%.5f_%.5f_%s.pdf",DATE.Data(),kineLabel.Data(),ctauLo,ctauHi,cutName.Data()));
    else if(ctauCut==-1)c_decayL_v3->SaveAs(Form("figs/decayL_v3/%s/decayL_v3_%s_ctau_%.5f_%s.pdf",DATE.Data(),kineLabel.Data(),ctauLo,cutName.Data()));
    else if(ctauCut== 1)c_decayL_v3->SaveAs(Form("figs/decayL_v3/%s/decayL_v3_%s_ctau_%.5f_%s.pdf",DATE.Data(),kineLabel.Data(),ctauHi,cutName.Data()));
    else c_decayL_v3->SaveAs(Form("figs/decayL_v3/%s/decayL_v3_%s_Inc.pdf",DATE.Data(),kineLabel.Data()));
    ...
    c_mass->Update();
    if(ctauCut==0)c_mass->SaveAs(Form("figs/mass_dist_v3/%s/MassDist_%s_ctau_%.5f_%.5f_%s.pdf",DATE.Data(),kineLabel.Data(),ctauLo,ctauHi,cutName.Data()));
    else if(ctauCut==-1)c_mass->SaveAs(Form("figs/mass_dist_v3/%s/MassDist_%s_ctau_%.5f_%s.pdf",DATE.Data(),kineLabel.Data(),ctauLo,cutName.Data()));
    else if(ctauCut==1)c_mass->SaveAs(Form("figs/mass_dist_v3/%s/MassDist_%s_ctau_%.5f_%s.pdf",DATE.Data(),kineLabel.Data(),ctauHi,cutName.Data()));
    else c_mass->SaveAs(Form("figs/mass_dist_v3/%s/MassDist_%s_Inc.pdf",DATE.Data(),kineLabel.Data()));
...
if(ctauCut==0) c_qq_1->SaveAs(Form("figs/q_vector_v3/%s/c_qqa_%s_Eff%d_Acc%d_PtW%d_TnP%d_ctau_%.5f_%.5f_%s.pdf",DATE.Data(),kineLabel.Data(),fEffW,fAccW,isPtW,isTnP,ctauLo,ctauHi,cutName.Data()));
else if(ctauCut==-1)c_qq_1->SaveAs(Form("figs/q_vector_v3/%s/c_qqa_%s_Eff%d_Acc%d_PtW%d_TnP%d_ctau_%.5f_%s.pdf",DATE.Data(),kineLabel.Data(),fEffW,fAccW,isPtW,isTnP,ctauLo,cutName.Data()));
else if(ctauCut==1)c_qq_1->SaveAs(Form("figs/q_vector_v3/%s/c_qqa_%s_Eff%d_Acc%d_PtW%d_TnP%d_ctau_%.5f_%s.pdf",DATE.Data(),kineLabel.Data(),fEffW,fAccW,isPtW,isTnP,ctauHi,cutName.Data()));
else c_qq_1->SaveAs(Form("figs/q_vector_v3/%s/c_qqa_%s_Eff%d_Acc%d_PtW%d_TnP%d_Inc.pdf",DATE.Data(),kineLabel.Data(),fEffW,fAccW,isPtW,isTnP));
...
leg_v2_2->Draw("same");
if(ctauCut==0) c_qq_2->SaveAs(Form("figs/q_vector_v3/%s/c_qaqb_%s_Eff%d_Acc%d_PtW%d_TnP%d_ctau_%.5f_%.5f_%s.pdf",DATE.Data(),kineLabel.Data(),fEffW,fAccW,isPtW,isTnP,ctauLo,ctauHi,cutName.Data()));
else if(ctauCut==-1)c_qq_2->SaveAs(Form("figs/q_vector_v3/%s/c_qaqb_%s_Eff%d_Acc%d_PtW%d_TnP%d_ctau_%.5f_%s.pdf",DATE.Data(),kineLabel.Data(),fEffW,fAccW,isPtW,isTnP,ctauLo,cutName.Data()));
else if(ctauCut==1)c_qq_2->SaveAs(Form("figs/q_vector_v3/%s/c_qaqb_%s_Eff%d_Acc%d_PtW%d_TnP%d_ctau_%.5f_%s.pdf",DATE.Data(),kineLabel.Data(),fEffW,fAccW,isPtW,isTnP,ctauHi,cutName.Data()));
else c_qq_2->SaveAs(Form("figs/q_vector_v3/%s/c_qaqb_%s_Eff%d_Acc%d_PtW%d_TnP%d_Inc.pdf",DATE.Data(),kineLabel.Data(),fEffW,fAccW,isPtW,isTnP));
...
leg_v2_3->Draw("same");
if(ctauCut==0) c_qq_3->SaveAs(Form("figs/q_vector_v3/%s/c_qaqc_%s_Eff%d_Acc%d_PtW%d_TnP%d_ctau_%.5f_%.5f_%s.pdf",DATE.Data(),kineLabel.Data(),fEffW,fAccW,isPtW,isTnP,ctauLo,ctauHi,cutName.Data()));
else if(ctauCut==-1)c_qq_3->SaveAs(Form("figs/q_vector_v3/%s/c_qaqc_%s_Eff%d_Acc%d_PtW%d_TnP%d_ctau_%.5f_%s.pdf",DATE.Data(),kineLabel.Data(),fEffW,fAccW,isPtW,isTnP,ctauLo,cutName.Data()));
else if(ctauCut== 1)c_qq_3->SaveAs(Form("figs/q_vector_v3/%s/c_qaqc_%s_Eff%d_Acc%d_PtW%d_TnP%d_ctau_%.5f_%s.pdf",DATE.Data(),kineLabel.Data(),fEffW,fAccW,isPtW,isTnP,ctauHi,cutName.Data()));
else c_qq_3->SaveAs(Form("figs/q_vector_v3/%s/c_qaqc_%s_Eff%d_Acc%d_PtW%d_TnP%d_Inc.pdf",DATE.Data(),kineLabel.Data(),fEffW,fAccW,isPtW,isTnP));
...
leg_v2_4->Draw("same");
if(ctauCut==0) c_qq_4->SaveAs(Form("figs/q_vector_v3/%s/c_qbqc_%s_Eff%d_Acc%d_PtW%d_TnP%d_ctau_%.5f_%.5f_%s.pdf",DATE.Data(),kineLabel.Data(),fEffW,fAccW,isPtW,isTnP,ctauLo,ctauHi,cutName.Data()));
else if(ctauCut==-1)c_qq_4->SaveAs(Form("figs/q_vector_v3/%s/c_qbqc_%s_Eff%d_Acc%d_PtW%d_TnP%d_ctau_%.5f_%s.pdf",DATE.Data(),kineLabel.Data(),fEffW,fAccW,isPtW,isTnP,ctauLo,cutName.Data()));
else if(ctauCut==1)c_qq_4->SaveAs(Form("figs/q_vector_v3/%s/c_qbqc_%s_Eff%d_Acc%d_PtW%d_TnP%d_ctau_%.5f_%s.pdf",DATE.Data(),kineLabel.Data(),fEffW,fAccW,isPtW,isTnP,ctauHi,cutName.Data()));
else c_qq_4->SaveAs(Form("figs/q_vector_v3/%s/c_qbqc_%s_Eff%d_Acc%d_PtW%d_TnP%d_Inc.pdf",DATE.Data(),kineLabel.Data(),fEffW,fAccW,isPtW,isTnP));

TFile *wf; 
if(ctauCut==0) wf = new TFile(Form("roots/v3mass_hist/%s/Jpsi_%s_Eff%d_Acc%d_PtW%d_TnP%d_ctau_%.5f_%.5f_%s.root",DATE.Data(),kineLabel.Data(),fEffW,fAccW,isPtW,isTnP,ctauLo,ctauHi,cutName.Data()),"recreate");
else if(ctauCut==-1) wf = new TFile(Form("roots/v3mass_hist/%s/Jpsi_%s_Eff%d_Acc%d_PtW%d_TnP%d_ctau_%.5f_%s.root",DATE.Data(),kineLabel.Data(),fEffW,fAccW,isPtW,isTnP,ctauLo,cutName.Data()),"recreate");
else if(ctauCut==1) wf = new TFile(Form("roots/v3mass_hist/%s/Jpsi_%s_Eff%d_Acc%d_PtW%d_TnP%d_ctau_%.5f_%s.root",DATE.Data(),kineLabel.Data(),fEffW,fAccW,isPtW,isTnP,ctauHi,cutName.Data()),"recreate");
else wf = new TFile(Form("roots/v3mass_hist/%s/Jpsi_%s_Eff%d_Acc%d_PtW%d_TnP%d_Inc.root",DATE.Data(),kineLabel.Data(),fEffW,fAccW,isPtW,isTnP),"recreate");
wf->cd();
h_v2_final->Write();
h_decayL_v3->Write();
g_mass->Write();
