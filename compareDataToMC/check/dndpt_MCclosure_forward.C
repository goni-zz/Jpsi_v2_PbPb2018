...
void dndpt_MCclosure_forward(int PR=0, int WRITE=1) {
  ...
  double ptBinMC[nPtBinsMC+1]={3.0,4.0,5.0,6.5,7.5,8.0,9,10.0,11,12.0,20.0,50.0};
  const int nYBins=6;
  ...
  TFile *fPtW1 = new TFile("WeightedFcN_fit/ratioDataMC_AA_Jpsi_DATA_Forward_y_211208.root","read");
  //TFile *fPtW2 = new TFile("../WeightedFcN_fit/ratioDataMC_AA_Jpsi_DATA_All_y_211208.root","read");
  TFile *fPtW2 = new TFile("WeightedFcN_fit/ratioDataMC_AA_Jpsi_DATA_All_y_211208.root","read");
  ...
  for(int i=0; i<nEvt; i++){
    ...
    for(int j=0; j<nDimu; j++){
      ...
      wt1 = fptw1->Eval(pt[j]);
      wt2 = fptw2->Eval(pt[j]);
      //if(pt[j]>3&&pt[j]<4) cout<<wt1<<endl;
      //wt1 = getAccWeight(hptw1, pt[j]);
      //	  if(pt[j]>6.5 && fabs(y[j])<2.4){
