...
//  f1 = new TFile("../skimmedFiles/OniaRooDataSet_isMC0_JPsi_w_Effw0_Accw0_PtW1_TnP1_20210111.root");
  ...
  f1 = new TFile(Form("../../skimmedFiles/OniaRooDataSet_isMC0_JPsi_%sw_Effw%d_Accw%d_PtW%d_TnP%d_210915.root",fname.Data(),1,1,1,1));
...
//  RooBinning bins(nBins, ws->var("ctau3DErr")->getMin(), ws->var("ctau3DErr")->getMax());
  ...
  for(int i=20; i<hSig->GetNbinsX(); i++){
    ...
    //if(hSig->GetBinContent(i+1)<1&&hSig->GetBinContent(i+2)<1&&hSig->GetBinContent(i+3)<1&&hSig->GetBinContent(i+4)<1){
    //if(hSig->GetBinContent(i+1)<1){//pt7-7.5
