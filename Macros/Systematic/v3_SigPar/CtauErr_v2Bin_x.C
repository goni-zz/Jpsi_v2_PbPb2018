    ...
    )
{
  ...
  gSystem->mkdir(Form("roots/2DFit_%s/CtauErr_v2Bins_x",DATE.Data()),kTRUE);
  gSystem->mkdir(Form("figs/2DFit_%s/CtauErr_v2Bins_x",DATE.Data()),kTRUE);
...
//  RooBinning bins(nBins, ctauErrLow, ctauErrHigh);
  ...
  c_B->Update();
  c_B->SaveAs(Form("figs/2DFit_%s/CtauErr_v2Bins_x/ctauErr_%s_%sw_Effw%d_Accw%d_PtW%d_TnP%d_v2_%.2f.pdf", DATE.Data(), kineLabel.Data(), fname.Data(), fEffW, fAccW, isPtW, isTnP, v2));

  TFile *outFile = new TFile(Form("roots/2DFit_%s/CtauErr_v2Bins_x/CtauErrResult_%s_%sw_Effw%d_Accw%d_PtW%d_TnP%d_v2_%.2f.root", DATE.Data(), kineLabel.Data(), fname.Data(), fEffW, fAccW, isPtW, isTnP, v2),"recreate");
  dataw_Bkg->Write();
  dataw_Sig->Write();
