...
void MassFit_FixPar_Data_v2Bins_x(
    double v2=0,
    ...
    )
{
  ...
  gSystem->mkdir(Form("roots/2DFit_%s/Mass_v2Bins_x",DATE.Data()),kTRUE);
  gSystem->mkdir(Form("figs/2DFit_%s/Mass_v2Bins_x",DATE.Data()),kTRUE);
...
//  f1 = new TFile("/Users/hwan/tools/2019/CMS/JPsi/Jpsi_v2_PbPb2018/skimmedFiles/vnCut/OniaRooDataSet_isMC0_JPsi_pt3.0-4.5_y1.6-2.4_muPt0.0_centrality20-120_m2.6-3.5_OS_Effw0_Accw0_PtW1_TnP1_211110.root");
  ...
  else if(v2==0.3&&ptLow==3&&ptHigh==6.5) sigma_index=3;
  else if(v2==-0.9&&ptLow==3&&ptHigh==6.5) sigma_index=3;
  //else if(v2==1.5&&ptLow==3&&ptHigh==6.5) sigma_index=3;
...
//
  ...
  TFile* outFile;
  outFile = new TFile(Form("roots/2DFit_%s/Mass_v2Bins_x/Mass_FixedFitResult_%s_%sw_Effw%d_Accw%d_PtW%d_TnP%d_v2_%.2f.root", DATE.Data(), kineLabel.Data(), fname.Data(), fEffW, fAccW, isPtW, isTnP, v2),"recreate");
  c_A->SaveAs(Form("figs/2DFit_%s/Mass_v2Bins_x/Mass_Fixed_%s_%sw_Effw%d_Accw%d_PtW%d_TnP%d_v2_%.2f.pdf", DATE.Data(), kineLabel.Data(), fname.Data(), fEffW, fAccW, isPtW, isTnP, v2));
  pdfMASS_Tot->Write();
  pdfMASS_bkg->Write();
  datasetMass->Write();
  ws->Write();
