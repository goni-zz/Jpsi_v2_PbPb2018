...
//            hNP->SetBinError(iv2+1,errprop_npr/2.0);
    ...
    TFile *outFile;
    outFile = new TFile(Form("./SigPar_n/v3_D0method_%s_n.root",kineLabel.Data()),"recreate");
    outFile->cd();
    hOutPR->Write();
    hOutNP->Write();
...
double getFrac(float ptLow, float ptHigh, float yLow, float yHigh, int cLow, int cHigh, double v2) {
    ...
    //TFile* inf = new TFile(Form("./roots/2DFit_0_180/Final_v2Bins_n/2DFitResult_%s_PRw_Effw1_Accw1_PtW1_TnP1_v2_%.2f.root", kineLabel.Data(),v2));
