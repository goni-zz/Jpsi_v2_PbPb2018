...
void D0_method_pt_errmod_alpha(
        ...
        )
{
    ...
    TH1D* hPR = new TH1D("hPR",";v_{2};",nV2Bins,v2Bin);
    TH1D* hNP = new TH1D("hNP",";v_{2};",nV2Bins,v2Bin);
    TH1D* hPRbfr = new TH1D("hPRbfr",";v_{2};b fraction",nV2Bins,v2Bin);
    TH1D* hNPbfr = new TH1D("hNPbfr",";v_{2};b fraction",nV2Bins,v2Bin);
    TF1* f1;
...
//            hNP->SetBinError(iv2+1,errprop_npr/2.0);
    ...
    TFile *outFile;
    outFile = new TFile(Form("./SigPar_alpha/v3_D0method_%s_alpha.root",kineLabel.Data()),"recreate");
    outFile->cd();
    hOutPR->Write();
    hOutNP->Write();
...
double getFrac(float ptLow, float ptHigh, float yLow, float yHigh, int cLow, int cHigh, double v2) {
	...
	TFile* inf = new TFile(Form("../220108_v3_Comb3_pT_Final/roots/2DFit_10_60/Final_v2Bins/2DFitResult_%s_PRw_Effw1_Accw1_PtW1_TnP1_v2_%.2f.root", kineLabel.Data(),v2));
    //TFile* inf = new TFile(Form("./roots/2DFit_0_180/Final_v2Bins_alpha/2DFitResult_%s_PRw_Effw1_Accw1_PtW1_TnP1_v2_%.2f.root", kineLabel.Data(),v2));
