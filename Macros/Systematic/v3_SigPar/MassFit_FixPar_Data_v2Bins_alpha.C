...
void MassFit_FixPar_Data_v2Bins_alpha(
    double v2=0,
    ...
    )
{
  ...
  gSystem->mkdir(Form("roots/2DFit_%s/Mass_v2Bins_alpha",DATE.Data()),kTRUE);
  gSystem->mkdir(Form("figs/2DFit_%s/Mass_v2Bins_alpha",DATE.Data()),kTRUE);
...
//  else if(yLow!=0)drawText(Form("%.1f < |y^{#mu#mu}| < %.1f",yLow, yHigh), text_x,text_y-y_diff,text_color,text_size);
  ...
  TFile* outFile;
  outFile = new TFile(Form("roots/2DFit_%s/Mass_v2Bins_alpha/Mass_FixedFitResult_%s_%sw_Effw%d_Accw%d_PtW%d_TnP%d_v2_%.2f.root", DATE.Data(), kineLabel.Data(), fname.Data(), fEffW, fAccW, isPtW, isTnP, v2),"recreate");
  c_A->SaveAs(Form("figs/2DFit_%s/Mass_v2Bins_alpha/Mass_Fixed_%s_%sw_Effw%d_Accw%d_PtW%d_TnP%d_v2_%.2f.pdf", DATE.Data(), kineLabel.Data(), fname.Data(), fEffW, fAccW, isPtW, isTnP, v2));
  pdfMASS_Tot->Write();
  pdfMASS_bkg->Write();
  datasetMass->Write();
  ws->Write();
