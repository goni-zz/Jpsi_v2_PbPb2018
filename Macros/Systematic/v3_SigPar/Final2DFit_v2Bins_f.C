    ...
    )
{
  ...
  gSystem->mkdir(Form("roots/2DFit_%s/Final_v2Bins_f",DATE.Data()),kTRUE);
  gSystem->mkdir(Form("figs/2DFit_%s/Final_v2Bins_f", DATE.Data()),kTRUE);
  ...
  fCBkg = new TFile(Form("roots/2DFit_%s/CtauBkg_v2Bins_f/CtauBkgResult_%s_%sw_Effw%d_Accw%d_PtW%d_TnP%d_v2_%.2f.root", DATE.Data(), kineLabel.Data(), fname.Data(), fEffW, fAccW, isPtW, isTnP, v2));
  ...
  ws->var("ctau3DRes")->setRange(-6, 6); // -6, 6
  ws->var("ctau3DRes")->setRange("ctauRange", -6, 6); // -6, 6
  ws->var("mass")->Print();
  ws->var("ctau3D")->Print();
  ...
  else if(ptLow==6.5&&ptHigh==50&&cLow==0&&cHigh==40 && v2 == 0.0) ws->factory("b_Jpsi[0.0000001, 0.00000000001, 0.40]");//NP fraction for Sig
  ...
  else if(ptLow==6.5&&ptHigh==50 && cLow==80 && cHigh==180 && v2 == 0.3) ws->factory("b_Jpsi[0.2962, 0.287, 0.330]");//NP fraction for Sig
  //else if(ptLow==6.5&&ptHigh==50 && cLow==80 && cHigh==180 && v2 == 0.3) ws->factory("b_Jpsi[0.60, 1e-6, 1.0]");//NP fraction for Sig
  else if(ptLow==6.5&&ptHigh==50 && cLow==80 && cHigh==180 && v2 == -1.2) ws->factory("b_Jpsi[0.35, 1e-6, 1.0]");//NP fraction for Sig
  else if(ptLow==6.5&&ptHigh==50 && cLow==80 && cHigh==180 && v2 == -1.5) ws->factory("b_Jpsi[0.2962, 0.287, 0.400]");//NP fraction for Sig
  //else if(ptLow==6.5&&ptHigh==50 && cLow==80 && cHigh==180 && v2 == -0.9) ws->factory("b_Jpsi[0.90, 0.32, 1.0]");//NP fraction for Sig
  else if(ptLow==6.5&&ptHigh==50) ws->factory("b_Jpsi[0.30, 1e-6, 1.0]");//NP fraction for Sig
  //else if(ptLow==6.5&&ptHigh==50) ws->factory("b_Jpsi[0.30, 1e-6, 1.0]");//NP fraction for Sig
  ...
  RooFitResult* fitResult = ws->pdf("pdfCTAUMASS_Tot")->fitTo(*dsToFit, Extended(kTRUE), ExternalConstraints(*ws->set("ConstrainPdfList")), NumCPU(nCPU), SumW2Error(true), PrintLevel(3), Save());
  ...
  ofstream of1(Form("figs/2DFit_%s/Final_v2Bins_f/2DFit_Ctau_%s_%sw_Effw%d_Accw%d_PtW%d_TnP%d_Left.txt", DATE.Data(), kineLabel.Data(), fname.Data(), fEffW, fAccW, isPtW, isTnP));
  ofstream of2(Form("figs/2DFit_%s/Final_v2Bins_f/2DFit_Ctau_%s_%sw_Effw%d_Accw%d_PtW%d_TnP%d_Righ.txt", DATE.Data(), kineLabel.Data(), fname.Data(), fEffW, fAccW, isPtW, isTnP));
  ofstream of3(Form("figs/2DFit_%s/Final_v2Bins_f/2DFit_Ctau_%s_%sw_Effw%d_Accw%d_PtW%d_TnP%d_Cent.txt", DATE.Data(), kineLabel.Data(), fname.Data(), fEffW, fAccW, isPtW, isTnP));
  ...
  c_G->Update();
  c_G->SaveAs(Form("figs/2DFit_%s/Final_v2Bins_f/2DFit_Ctau_%s_%sw_Ef%d_Ac%d_Pt%d_TnP%d_v2_%.2f.pdf", DATE.Data(), kineLabel.Data(), fname.Data(), fEffW, fAccW, isPtW, isTnP, v2));
  //c_G->SaveAs(Form("figs/2DFit_%s/Final_v2Bins_f/2DFit_Ctau_%s_%sw_Effw%d_Accw%d_PtW%d_TnP%d_v2_%.2f.pdf", DATE.Data(), kineLabel.Data(), fname.Data(), fEffW, fAccW, isPtW, isTnP, v2));
  c_H->Update();
  c_H->SaveAs(Form("figs/2DFit_%s/Final_v2Bins_f/2DFit_Mass_%s_%sw_Ef%d_Ac%d_Pt%d_TnP%d_v2_%.2f.pdf", DATE.Data(), kineLabel.Data(), fname.Data(), fEffW, fAccW, isPtW, isTnP, v2));
  //c_H->SaveAs(Form("figs/2DFit_%s/Final_v2Bins_f/2DFit_Mass_%s_%sw_Effw%d_Accw%d_PtW%d_TnP%d_v2_%.2f.pdf", DATE.Data(), kineLabel.Data(), fname.Data(), fEffW, fAccW, isPtW, isTnP, v2));
  ...
  fitResult->Print("v");
  TFile *outFile = new TFile(Form("roots/2DFit_%s/Final_v2Bins_f/2DFitResult_%s_%sw_Effw%d_Accw%d_PtW%d_TnP%d_v2_%.2f.root", DATE.Data(), kineLabel.Data(), fname.Data(), fEffW, fAccW, isPtW, isTnP, v2),"recreate");
  //ws->Write();
  outFile->cd();
  outh->Write();
  outh1->Write();
