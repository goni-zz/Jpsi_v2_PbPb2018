    ...
    )
{
  ...
  gSystem->mkdir(Form("roots/2DFit_%s/CtauErr_v2Bins_f",DATE.Data()),kTRUE);
  gSystem->mkdir(Form("figs/2DFit_%s/CtauErr_v2Bins_f",DATE.Data()),kTRUE);
...
//  RooBinning bins(nBins, ctauErrLow, ctauErrHigh);
  ...
  if(cLow==20 && cHigh==120 && ptLow == 3.0 && ptHigh == 6.5 && v2 == -0.3) ctauErrMax = 0.140; 
  ...
  if(cLow==20 && cHigh==120 && ptLow == 9.0 && ptHigh == 12.0 && v2 == 0.0) ctauErrMax = 0.089; 
  ...
  if(cLow==20 && cHigh==120 && ptLow == 12.0 && ptHigh == 15.0 && v2 == 0.6) ctauErrMax = 0.0700; 
  ...
  c_B->Update();
  c_B->SaveAs(Form("figs/2DFit_%s/CtauErr_v2Bins_f/ctauErr_%s_%sw_Effw%d_Accw%d_PtW%d_TnP%d_v2_%.2f.pdf", DATE.Data(), kineLabel.Data(), fname.Data(), fEffW, fAccW, isPtW, isTnP, v2));

  TFile *outFile = new TFile(Form("roots/2DFit_%s/CtauErr_v2Bins_f/CtauErrResult_%s_%sw_Effw%d_Accw%d_PtW%d_TnP%d_v2_%.2f.root", DATE.Data(), kineLabel.Data(), fname.Data(), fEffW, fAccW, isPtW, isTnP, v2),"recreate");
  dataw_Bkg->Write();
  dataw_Sig->Write();
