...
//  ws->factory("lambdaDSS_Bkg[0.5, 1e-6, 1.]");}
  ...
  if(cLow==80&&cHigh==100) ctauMin = -1.1;
  else if(cLow==100&&cHigh==180) { ctauMin=-1.; ctauMax=2.4; }
...
//  hTot->SetMarkerColor(kRed);
//  hTot->Draw();
//  TH1D* hFit = (TH1D*)ws->data("dataToFit")->createHistogram(("hFit"), *ws->var("ctau3D"),Binning(nCtauBins,ctauLow,ctauHigh));
