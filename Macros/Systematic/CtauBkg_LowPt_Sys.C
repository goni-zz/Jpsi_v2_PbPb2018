...
void CtauBkg_LowPt_Sys(
    float ptLow=9, float ptHigh=10,
    float yLow=0, float yHigh=2.4,
    int cLow=20, int cHigh=120,float muPtCut=0.0,
    ...
    )
{
  ...
  //
  //ws->factory("b_Bkg[0.674, 0.62, 0.8]");//NP fraction for bkg
  //ws->factory("fDFSS[0.7610, 0.750, 1.]");
  //ws->factory("fDLIV[0.9999, 0.9444, 1.]");
  //ws->factory("lambdaDDS_Bkg[0.5010, 0.499, 1.]");
  //ws->factory("lambdaDF_Bkg[0.0450, 0.0205, 0.0515]");
  //ws->factory("lambdaDSS_Bkg[0.3342, 0.284, 1.]");

  ws->factory("b_Bkg[0.96, 0., 0.99]");//NP fraction for bkg
  ws->factory("fDFSS[0.4521, 0., 1.]");
  ws->factory("fDLIV[0.9811, 0., 1.]");
  ws->factory("lambdaDDS_Bkg[0.39, 1e-6, 1.]");
  ws->factory("lambdaDF_Bkg1[0.31, 1e-6, 1.]");
  ws->factory("lambdaDF_Bkg2[0.0241, 1e-6, 1.]");
  ws->factory("lambdaDF_Bkg3[0.0841, 1e-6, 1.]");
  ws->factory("lambdaDSS_Bkg1[0.0375, 1e-6, 1.]");
  ws->factory("lambdaDSS_Bkg2[0.481, 1e-6, 1.]");
  ws->factory("lambdaDSS_Bkg3[0.115, 1e-6, 1.]");
  ws->factory("fDSS12[0.81, 0., 1.]");
  ws->factory("fDSS[0.81, 0., 1.]");
  ws->factory("fDF12[0.81, 0., 1.]");
  ws->factory("fDF[0.81, 0., 1.]");
  ...
  ws->factory(Form("Decay::%s(%s, %s, %s, RooDecay::SingleSided)", "pdfCTAUDSS3", "ctau3D", "lambdaDSS_Bkg3", "pdfCTAURES"));
  ...
  ws->factory(Form("Decay::%s(%s, %s, %s, RooDecay::Flipped)", "pdfCTAUDF3", "ctau3D", "lambdaDF_Bkg3", "pdfCTAURES"));
  ws->factory(Form("Decay::%s(%s, %s, %s, RooDecay::DoubleSided)", "pdfCTAUDDS", "ctau3D", "lambdaDDS_Bkg", "pdfCTAURES"));
  ws->factory(Form("SUM::%s(%s*%s, %s)", "pdfCTAUDSS12", "fDSS12", "pdfCTAUDSS1", "pdfCTAUDSS2"));
  ws->factory(Form("SUM::%s(%s*%s, %s)", "pdfCTAUDSS", "fDSS", "pdfCTAUDSS12", "pdfCTAUDSS3"));
  ws->factory(Form("SUM::%s(%s*%s, %s)", "pdfCTAUDF12", "fDF12", "pdfCTAUDF1", "pdfCTAUDF2"));
  ws->factory(Form("SUM::%s(%s*%s, %s)", "pdfCTAUDF", "fDF", "pdfCTAUDF12", "pdfCTAUDF3"));
...
//  TH1D* hTot = (TH1D*)ws->data("dataw_Bkg")->createHistogram(("hTot"), *ws->var("ctau3D"),Binning(nCtauBins,ctauLow,ctauHigh));
  ...
  TH1D* hTot = (TH1D*)ws->data("dataw_Bkg")->createHistogram(("hTot"), *ws->var("ctau3D"),Binning(nCtauBins,ctauLow,ctauHigh));
  cout<<"\n Search Min"<<endl;
  for(int i=10; i<hTot->GetNbinsX()/2; i++){
      cout<<"Bin: "<<i<<", Ctau: "<<hTot->GetBinLowEdge(i)<<", Events: "<<hTot->GetBinContent(i)<<endl;
    if(hTot->GetBinContent(i)<=0&&hTot->GetBinContent(i+1)<=0&&hTot->GetBinContent(i+2)>=1&&hTot->GetBinContent(i+3)>=1){
    //if(hTot->GetBinContent(i)<1&&hTot->GetBinContent(i+1)<1&&hTot->GetBinContent(i+2)>1){
  ...
  double ctauMax=3;
  cout<<"\n Search Max"<<endl;
  for(int i=45; i<hTot->GetNbinsX(); i++){
      cout<<"Bin: "<<i<<", Ctau: "<<hTot->GetBinLowEdge(i)+hTot->GetBinWidth(i)<<", Events: "<<hTot->GetBinContent(i)<<endl;
    if(hTot->GetBinContent(i)>1&&hTot->GetBinContent(i+1)<1&&hTot->GetBinContent(i+2)<1){
      cout<<"##### i(Max): "<<i<<", Ctau:  "<<hTot->GetBinLowEdge(i)+hTot->GetBinWidth(i)<<", Events: "<<hTot->GetBinContent(i)<<endl;
      cout<<"##### i: "<<i+1<<", Ctau:  "<<hTot->GetBinLowEdge(i+1)+hTot->GetBinWidth(i+1)<<", Events: "<<hTot->GetBinContent(i+1)<<endl;
      cout<<"##### i: "<<i+2<<", Ctau:  "<<hTot->GetBinLowEdge(i+2)+hTot->GetBinWidth(i+2)<<", Events: "<<hTot->GetBinContent(i+2)<<endl;
      ctauMax = hTot->GetBinLowEdge(i-1)+hTot->GetBinWidth(i-1);
      break;}
    else { ctauMax = hTot->GetBinLowEdge(i); }
  }

  if (ptLow==3&&ptHigh==4.5) { ctauMax=3; }
  else if (ptLow==6&&ptHigh==7) {ctauMin=-1.8; ctauMax=3; }

  //hTot->Draw();
  //c1->SaveAs("../figs/2DFit_20210208/CtauBkg/CtauBkg.pdf");
  //hTot->SaveAs("../figs/2DFit_20210208/CtauBkg/CtauBkg.root");

  TCanvas* c_E =  new TCanvas("canvas_E","My plots",1,1,550,520);
  c_E->cd();
  ...
  //ws->pdf("pdfCTAU_Bkg_Tot")->setNormRange("ctauRange");
  ...
  drawText(Form("#lambdaDF1_{Bkg} = %.4f #pm %.4f", ws->var("lambdaDF_Bkg1")->getVal(), ws->var("lambdaDF_Bkg1")->getError() ),text_x+0.45,text_y-y_diff*7,text_color,text_size);
  drawText(Form("#lambdaDF2_{Bkg} = %.4f #pm %.4f", ws->var("lambdaDF_Bkg2")->getVal(), ws->var("lambdaDF_Bkg2")->getError() ),text_x+0.45,text_y-y_diff*8,text_color,text_size);
  drawText(Form("#lambdaDF3_{Bkg} = %.4f #pm %.4f", ws->var("lambdaDF_Bkg3")->getVal(), ws->var("lambdaDF_Bkg3")->getError() ),text_x+0.45,text_y-y_diff*9,text_color,text_size);
  drawText(Form("#lambdaDSS1_{Bkg} = %.4f #pm %.4f", ws->var("lambdaDSS_Bkg1")->getVal(), ws->var("lambdaDSS_Bkg1")->getError() ),text_x+0.45,text_y-y_diff*10,text_color,text_size);
  drawText(Form("#lambdaDSS2_{Bkg} = %.4f #pm %.4f", ws->var("lambdaDSS_Bkg2")->getVal(), ws->var("lambdaDSS_Bkg2")->getError() ),text_x+0.45,text_y-y_diff*11,text_color,text_size);
  drawText(Form("#lambdaDSS3_{Bkg} = %.4f #pm %.4f", ws->var("lambdaDSS_Bkg3")->getVal(), ws->var("lambdaDSS_Bkg3")->getError() ),text_x+0.45,text_y-y_diff*12,text_color,text_size);
