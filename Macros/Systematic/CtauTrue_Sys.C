    ...
    )
{
  ...
  ws->factory("lambdaDSS[0.43, 1e-6, 1.0]");
  ws->factory("lambdaDSS2[0.82, 1e-6, 1.0]");
  ws->factory("lambdaDSS3[.46, 1e-6, 1.0]");
  ws->factory("lambdaDSS4[.29, 1e-6, 1.0]");
  ...
  //RooPlot* myPlot_D = wsmc->var("ctau3Dtrue")->frame(Bins(nBins), Range(-1, ctau3DTrueMax)); // bins
  ...
  drawText(Form("#lambdaDSS4 = %.4f #pm %.4f", ws->var("lambdaDSS4")->getVal(), ws->var("lambdaDSS4")->getError() ),text_x+0.5,text_y-y_diff*5,text_color,text_size);
