...
void CtauRes_SysTot(
    float ptLow=9.5, float ptHigh=11,
    ...
    )
{
  ...
  int nGauss = 3;
  ws->factory("One[1.0]");
  ws->factory("ctau1_CtauRes[0.]");  if (cLow==0&&cHigh==20) { ws->factory("s1_CtauRes[0.9, 0.8834, 1.]"); } else {ws->factory("s1_CtauRes[0.87, 0.7, 1.]");}
  ...
  ws->factory("rS21_CtauRes[1.7., 1., 10]");
  ws->factory("rS32_CtauRes[2.8, 1.0, 10]");
  ...
  //TH1D* hTot = (TH1D*)ws->data("dataw_Sig")->createHistogram(("hTot"), *ws->var("ctau3DRes"),Binning(nCtauResBins,ctauResLow,0));
  ...
  for(int i=0; i<hTot->GetNbinsX()/2; i++){
      cout<<"Bin: "<<i<<", CtauResolution: "<<hTot->GetBinLowEdge(i)<<", Events: "<<hTot->GetBinContent(i)<<endl;
    if(hTot->GetBinContent(i)<=0&&hTot->GetBinContent(i+1)<=0&&hTot->GetBinContent(i+2)>=1&&hTot->GetBinContent(i+3)>=1){
      cout<<"##### i: "<<i+1<<", CtauResolution:  "<<hTot->GetBinLowEdge(i+1)<<", Events: "<<hTot->GetBinContent(i+1)<<endl;
      cout<<"##### i(Min): "<<i+2<<", CtauResolution:  "<<hTot->GetBinLowEdge(i+2)<<", Events: "<<hTot->GetBinContent(i+2)<<endl;
      cout<<"##### i: "<<i+3<<", CtauResolution:  "<<hTot->GetBinLowEdge(i+3)<<", Events: "<<hTot->GetBinContent(i+3)<<endl;
      cout<<"##### i: "<<i+4<<", CtauResolution:  "<<hTot->GetBinLowEdge(i+4)<<", Events: "<<hTot->GetBinContent(i+4)<<endl;
      ctauResMin = hTot->GetBinLowEdge(i+2);
      break;}
  }
  if (cLow==0&&cHigh==20) ctauResMin=-7.1;
  else if (cLow==20&&cHigh==40) ctauResMin=-7.77778;
  else if (cLow==100&&cHigh==180) ctauResMin=-9.02778;
*/
  ...
  TH1D* hTot = (TH1D*)ws->data("ctauResCutDS")->createHistogram(("hTot"), *ws->var("ctau3DRes"),Binning(nCtauResBins,ctauResLow,0));
  for(int i=0; i<hTot->GetNbinsX()/2; i++){
      cout<<"Bin: "<<i<<", CtauResolution: "<<hTot->GetBinLowEdge(i)<<", Events: "<<hTot->GetBinContent(i)<<endl;
    if(hTot->GetBinContent(i)<=0&&hTot->GetBinContent(i+1)<=0&&hTot->GetBinContent(i+2)>=1&&hTot->GetBinContent(i+3)>=1){
      cout<<"##### i: "<<i+1<<", CtauResolution:  "<<hTot->GetBinLowEdge(i+1)<<", Events: "<<hTot->GetBinContent(i+1)<<endl;
