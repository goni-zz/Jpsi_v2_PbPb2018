    ...
    )
{
  ...
  gSystem->mkdir(Form("roots/2DFit_%s/CtauRes_v2Bins",DATE.Data()),kTRUE);
  gSystem->mkdir(Form("figs/2DFit_%s/CtauRes_v2Bins",DATE.Data()),kTRUE);
  ...
  //else if (ptLow==6.5&&ptHigh==7.5&&v2==-0.3) nGauss = 2;
  ...
  if(ptLow==6.5&&ptHigh==7&&cLow==20&&cHigh==120){
    ...
    ws->factory("rS21_CtauRes[1.62, 1.0, 5.0]");
    ws->factory("rS32_CtauRes[3.1, 1.0, 5.0]");}
  ...
  else if(v2==-1.2&&ptLow==20&&ptHigh==25&&cLow==20&&cHigh==120){
    ws->factory("s1_CtauRes[0.8, 0.5, 1.0]");
    ws->factory("rS21_CtauRes[2.1, 1.0, 10.0]");
    ws->factory("rS32_CtauRes[3.5, 1.0, 5.0]");}
  else if(ptLow==20&&ptHigh==25&&cLow==20&&cHigh==120){
    ws->factory("s1_CtauRes[0.5, 1e-3, 1.0]");
    ws->factory("rS21_CtauRes[1.1, 1.0, 10.0]");
    ws->factory("rS32_CtauRes[3.5, 1.0, 5.0]");}
  else if(v2==-2.7&&ptLow==20&&ptHigh==50&&cLow==20&&cHigh==120){
    ws->factory("s1_CtauRes[0.8, 1e-3, 1.0]");
    ws->factory("rS21_CtauRes[3.5, 2.5, 5.0]");
    ws->factory("rS32_CtauRes[3.5, 1.0, 5.0]");}
  else if(v2==2.1&&ptLow==20&&ptHigh==50&&cLow==20&&cHigh==120){
    ws->factory("s1_CtauRes[0.8, 1e-3, 1.0]");
    ws->factory("rS21_CtauRes[3.5, 1.5, 5.0]");
    ws->factory("rS32_CtauRes[3.5, 1.0, 5.0]");}
  ...
  c_C->Update();
  c_C->SaveAs(Form("figs/2DFit_%s/CtauRes_v2Bins/CtauRes_%s_%sw_Effw%d_Accw%d_PtW%d_TnP%d_v2_%.2f.pdf", DATE.Data(), kineLabel.Data(), fname.Data(), fEffW, fAccW, isPtW, isTnP, v2));

  TFile *outFile = new TFile(Form("roots/2DFit_%s/CtauRes_v2Bins/CtauResResult_%s_%sw_Effw%d_Accw%d_PtW%d_TnP%d_v2_%.2f.root", DATE.Data(), kineLabel.Data(), fname.Data(), fEffW, fAccW, isPtW, isTnP, v2),"recreate");
  ctauResModel->Write();
