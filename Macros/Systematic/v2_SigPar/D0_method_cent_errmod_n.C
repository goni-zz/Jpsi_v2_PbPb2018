    ...
    )
{
  ...
  c1->SaveAs(Form("./SigPar_n/plot_D0method_%s.png",kineLabel.Data()));
  c1->SaveAs(Form("./SigPar_n/plot_D0method_%s.pdf",kineLabel.Data()));
  ...
  c2->SaveAs(Form("./SigPar_n/plot_D0method_bfractions_%s.png",kineLabel.Data()));
  c2->SaveAs(Form("./SigPar_n/plot_D0method_bfractions_%s.pdf",kineLabel.Data()));
  ...
  TFile *outFile;
  outFile = new TFile(Form("./SigPar_n/v2_D0method_%s_n.root",kineLabel.Data()),"recreate");
  outFile->cd();
  hOutPR->Write();
  hOutNP->Write();
...
valErr getYield(float ptLow, float ptHigh, float yLow, float yHigh, int cLow, int cHigh, double v2) {
  TString kineLabel = getKineLabel(ptLow, ptHigh, yLow, yHigh, 0.0, cLow, cHigh);
  //TFile* inf = new TFile(Form("./roots/2DFit_10_60/Mass_v2Bins_n/Mass_FixedFitResult_%s_PRw_Effw1_Accw1_PtW1_TnP1_v2_%.2f.root", kineLabel.Data(),v2));
  TFile* inf = new TFile(Form("./roots/2DFit_0_180/Mass_v2Bins_n/Mass_FixedFitResult_%s_PRw_Effw1_Accw1_PtW1_TnP1_v2_%.2f.root", kineLabel.Data(),v2));
...
double getFrac(float ptLow, float ptHigh, float yLow, float yHigh, int cLow, int cHigh, double v2) {
  ...
  //TFile* inf = new TFile(Form("./roots/2DFit_10_60/Final_v2Bins_n/2DFitResult_%s_PRw_Effw1_Accw1_PtW1_TnP1_v2_%.2f.root", kineLabel.Data(),v2));
