    ...
    )
{
  ...
  gSystem->mkdir(Form("roots/2DFit_%s/CtauRes_v2Bins_f",DATE.Data()),kTRUE);
  gSystem->mkdir(Form("figs/2DFit_%s/CtauRes_v2Bins_f",DATE.Data()),kTRUE);
  ...
  c_C->Update();
  c_C->SaveAs(Form("figs/2DFit_%s/CtauRes_v2Bins_f/CtauRes_%s_%sw_Effw%d_Accw%d_PtW%d_TnP%d_v2_%.2f.pdf", DATE.Data(), kineLabel.Data(), fname.Data(), fEffW, fAccW, isPtW, isTnP, v2));

  TFile *outFile = new TFile(Form("roots/2DFit_%s/CtauRes_v2Bins_f/CtauResResult_%s_%sw_Effw%d_Accw%d_PtW%d_TnP%d_v2_%.2f.root", DATE.Data(), kineLabel.Data(), fname.Data(), fEffW, fAccW, isPtW, isTnP, v2),"recreate");
  ctauResModel->Write();
