...
//  f1 = new TFile("/Users/hwan/tools/2019/CMS/JPsi/Jpsi_v2_PbPb2018/skimmedFiles/vnCut/OniaRooDataSet_isMC0_JPsi_pt3.0-4.5_y1.6-2.4_muPt0.0_centrality20-120_m2.6-3.5_OS_Effw0_Accw0_PtW1_TnP1_211110.root");
  ...
  if(v2==-1.2&&ptLow==3&&ptHigh==4.5) sigma_index = 3;
  else if (v2==-0.6&&ptLow==15&&ptHigh==20) sigma_index=3;
  else if (v2==0.3&&ptLow==20&&ptHigh==50) sigma_index=3;
  ...
  if (ptLow==15&&ptHigh==50)  {
      ...
      NBkg_limit = 5.0e+04;
      NJpsi_limit = 5.0e+04;  }
  else if (v2==0.3&&cLow==100&&cHigh==180){
      NBkg_limit = 1.0e+04;
      NJpsi_limit = 3.0e+05;  }
...
//  else if(yLow!=0)drawText(Form("%.1f < |y^{#mu#mu}| < %.1f",yLow, yHigh), text_x,text_y-y_diff,text_color,text_size);
  ...
  drawText(Form("(#sigma2/#sigma1)_{J/#psi} = %.3f #pm %.4f",ws->var("x_A")->getVal(),ws->var("x_A")->getError()),text_x,text_y-y_diff*9,text_color,text_size);
