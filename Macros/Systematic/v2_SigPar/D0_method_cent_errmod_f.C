    ...
    )
{
  ...
  drawText(Form("Cent. %d - %d %%",cLow/2,cHigh/2),text_x,text_y-0.05,text_color,text_size);
  drawText(Form("Entries = %.f",hPR->GetEntries()),text_x,text_y-0.1,text_color,text_size);
  drawText(Form("Mean = %f #pm %f",hPR->GetMean(),hPR->GetMeanError()),text_x,text_y-0.15,text_color,text_size);
  c1->cd(2);
  c1->cd(2)->SetBottomMargin(0.2);
  //hNP->GetXaxis()->SetTitle("v_{2}");
  ...
  drawText(Form("Cent. %d - %d %%",cLow/2,cHigh/2),text_x,text_y-0.05,text_color,text_size);
  drawText(Form("Entries = %.f",hNP->GetEntries()),text_x,text_y-0.1,text_color,text_size);
  drawText(Form("Mean = %f #pm %f",hNP->GetMean(),hNP->GetMeanError()),text_x,text_y-0.15,text_color,text_size);
  c1->SaveAs(Form("./SigPar_f/plot_D0method_f_%s.png",kineLabel.Data()));
  c1->SaveAs(Form("./SigPar_f/plot_D0method_f_%s.pdf",kineLabel.Data()));
  ...
  hPRbfr->GetYaxis()->SetRangeUser(0,1);
  ...
  c2->SaveAs(Form("./SigPar_f/plot_D0method_bfractions_f_%s.png",kineLabel.Data()));
  c2->SaveAs(Form("./SigPar_f/plot_D0method_bfractions_f_%s.pdf",kineLabel.Data()));
  ...
  hPRNom->GetYaxis()->SetTitle("#frac{Nom}{f}");
  hPRNom->GetYaxis()->CenterTitle();
  hPRNom->GetYaxis()->SetRangeUser(0.9,1.1);
  hPRNom->Draw("P");
  jumSun(-2.8,1,2.8,1);
  c3->SaveAs("./Ratio_f.png");
  ...
  TFile *outFile;
  outFile = new TFile(Form("./SigPar_f/v2_D0method_%s_f.root",kineLabel.Data()),"recreate");
  outFile->cd();
  hOutPR->Write();
  hOutNP->Write();
...
valErr getYield(float ptLow, float ptHigh, float yLow, float yHigh, int cLow, int cHigh, double v2) {
  TString kineLabel = getKineLabel(ptLow, ptHigh, yLow, yHigh, 0.0, cLow, cHigh);
  //TFile* inf = new TFile(Form("./roots/2DFit_10_60/Mass_v2Bins_f/Mass_FixedFitResult_%s_PRw_Effw1_Accw1_PtW1_TnP1_v2_%.2f.root", kineLabel.Data(),v2));
  TFile* inf = new TFile(Form("./roots/2DFit_0_180/Mass_v2Bins_f/Mass_FixedFitResult_%s_PRw_Effw1_Accw1_PtW1_TnP1_v2_%.2f.root", kineLabel.Data(),v2));
...
double getFrac(float ptLow, float ptHigh, float yLow, float yHigh, int cLow, int cHigh, double v2) {
  ...
  //TFile* inf = new TFile(Form("./roots/2DFit_10_60/Final_v2Bins_f/2DFitResult_%s_PRw_Effw1_Accw1_PtW1_TnP1_v2_%.2f.root", kineLabel.Data(),v2));
  TFile* inf = new TFile(Form("../../21_11_11/roots/2DFit_0_180/Final_v2Bins/2DFitResult_%s_PRw_Effw1_Accw1_PtW1_TnP1_v2_%.2f.root", kineLabel.Data(),v2));
