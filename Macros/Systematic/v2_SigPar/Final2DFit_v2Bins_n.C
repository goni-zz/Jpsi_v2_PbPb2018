    ...
    )
{
  ...
  gSystem->mkdir(Form("roots/2DFit_%s/Final_v2Bins_n",DATE.Data()),kTRUE);
  gSystem->mkdir(Form("figs/2DFit_%s/Final_v2Bins_n", DATE.Data()),kTRUE);
  ...
  fCBkg = new TFile(Form("roots/2DFit_%s/CtauBkg_v2Bins_n/CtauBkgResult_%s_%sw_Effw%d_Accw%d_PtW%d_TnP%d_v2_%.2f.root", DATE.Data(), kineLabel.Data(), fname.Data(), fEffW, fAccW, isPtW, isTnP, v2));
...
//  else if(ptLow==6.5&&ptHigh==50&&cLow==100&&cHigh==180) ws->factory("b_Jpsi[0.31, 0.3, 1.]");//NP fraction for Sig
  ...
  //ws->factory(Form("N_Jpsi_n[%.3f, %.3f, %.3f]",njpsi, njpsi*0.95, njpsi*1.05));
  //ws->factory(Form("N_Jpsi_n[%.f, %.f, %.f]",njpsi, 0.,njpsi*1.05));

  //double nbkg = ws->var("N_Bkg")->getVal();
  //ws->factory(Form("N_Bkg_n[%.f, %.f, %.f]",nbkg, 0.,nbkg*1.05));
  ...
  ofstream of1(Form("figs/2DFit_%s/Final_v2Bins_n/2DFit_Ctau_%s_%sw_Effw%d_Accw%d_PtW%d_TnP%d_Left.txt", DATE.Data(), kineLabel.Data(), fname.Data(), fEffW, fAccW, isPtW, isTnP));
  ofstream of2(Form("figs/2DFit_%s/Final_v2Bins_n/2DFit_Ctau_%s_%sw_Effw%d_Accw%d_PtW%d_TnP%d_Righ.txt", DATE.Data(), kineLabel.Data(), fname.Data(), fEffW, fAccW, isPtW, isTnP));
  ofstream of3(Form("figs/2DFit_%s/Final_v2Bins_n/2DFit_Ctau_%s_%sw_Effw%d_Accw%d_PtW%d_TnP%d_Cent.txt", DATE.Data(), kineLabel.Data(), fname.Data(), fEffW, fAccW, isPtW, isTnP));
  ...
  c_G->Update();
  c_G->SaveAs(Form("figs/2DFit_%s/Final_v2Bins_n/2DFit_Ctau_%s_%sw_Effw%d_Accw%d_PtW%d_TnP%d_v2_%.2f.pdf", DATE.Data(), kineLabel.Data(), fname.Data(), fEffW, fAccW, isPtW, isTnP, v2));
  c_H->Update();
  c_H->SaveAs(Form("figs/2DFit_%s/Final_v2Bins_n/2DFit_Mass_%s_%sw_Effw%d_Accw%d_PtW%d_TnP%d_v2_%.2f.pdf", DATE.Data(), kineLabel.Data(), fname.Data(), fEffW, fAccW, isPtW, isTnP, v2));
  ...
  cor.Print();
  TFile *outFile = new TFile(Form("roots/2DFit_%s/Final_v2Bins_n/2DFitResult_%s_%sw_Effw%d_Accw%d_PtW%d_TnP%d_v2_%.2f.root", DATE.Data(), kineLabel.Data(), fname.Data(), fEffW, fAccW, isPtW, isTnP, v2),"recreate");
  //ws->Write();
  outFile->cd();
  outh->Write();
  outh1->Write();
