    ...
    )
{
  ...
  //fMass = new TFile(Form("../roots/2DFit_20210114/MassFitResult_%s_%s_%s.root",bCont.Data(),kineLabel.Data(),fOrd.Data()));
  ...
  ws->data("dataset_SPLOT")->plotOn(myPlot2_B,Name("dataCTAUERR_Tot"), MarkerSize(.7),  Binning(nBins), DataError(RooAbsData::SumW2));
  ws->pdf("pdfCTAUERR_Tot")->plotOn(myPlot2_B,Name("pdfCTAUERR_Tot"), LineColor(kGreen+1), Range(ctauErrLow,ctauErrHigh), LineWidth(2));
  ws->data("dataw_Sig")->plotOn(myPlot2_B,Name("dataHist_Sig"), MarkerSize(.7), LineColor(kRed+2), MarkerColor(kRed+2), Binning(nBins), DataError(RooAbsData::SumW2));
  ws->pdf("pdfCTAUERR_Jpsi")->plotOn(myPlot2_B,Name("pdfCTAUERR_Jpsi"),LineColor(kRed+2), LineWidth(2), Range(ctauErrLow,ctauErrHigh));
  ws->data("dataw_Bkg")->plotOn(myPlot2_B,Name("dataHist_Bkg"), MarkerSize(.7), LineColor(kBlue+2), MarkerColor(kBlue+2), Binning(nBins), DataError(RooAbsData::SumW2));
