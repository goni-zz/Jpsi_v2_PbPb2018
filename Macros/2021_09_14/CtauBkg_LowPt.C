    ...
    )
{
  ...
  ws->var("ctau3DErr")->setRange("ctauWindow",ctauErrMin, ctauErrMax);
  ...
  if(ptLow==3&&ptHigh==4.5&&cLow==20&&cHigh==120){
    ...
    ws->factory("b_Bkg[0.6203, 0., 1.]");//NP fraction for bkg
    ws->factory("fDFSS[0.4521, 0., 1.]");
    ws->factory("fDLIV[0.9811, 0., 1.]");
    ws->factory("lambdaDDS_Bkg[0.2735, 1e-6, 1.]");
    ws->factory("lambdaDF_Bkg1[0.1, 1e-6, 1.]");
    ws->factory("lambdaDF_Bkg2[0.1041, 1e-6, 1.]");
    ws->factory("lambdaDSS_Bkg1[0.3075, 1e-6, 1.]");
    ws->factory("lambdaDSS_Bkg2[0.4041, 1e-6, 1.]");
  ...
  //ws->factory(Form("AddModel::%s({%s , %s}, %s)", "pdfCTAUDSS", "pdfCTAUDSS1", "pdfCTAUDSS2", "fDSS12"));
  //ws->factory(Form("AddModel::%s({%s , %s}, %s)", "pdfCTAUDF", "pdfCTAUDF1", "pdfCTAUDF2", "fDF12"));
  ...
  drawText(Form("#lambdaDSS1_{Bkg} = %.4f #pm %.4f", ws->var("lambdaDSS_Bkg1")->getVal(), ws->var("lambdaDSS_Bkg1")->getError() ),text_x+0.45,text_y-y_diff*9,text_color,text_size);
  drawText(Form("#lambdaDSS2_{Bkg} = %.4f #pm %.4f", ws->var("lambdaDSS_Bkg2")->getVal(), ws->var("lambdaDSS_Bkg2")->getError() ),text_x+0.45,text_y-y_diff*10,text_color,text_size);
