    ...
    )
{
  ...
  gSystem->mkdir("../../roots/2DFit_20210208/Mass/", kTRUE);
  gSystem->mkdir("../../figs/2DFit_20210208/Mass/", kTRUE);
  ...
  //pt3-4.5
  //double paramsupper[8] = {0.4,    1.0,     2.9, 4.1, 1.0,     25.0};
  //double paramslower[8] = {0.01,   0.0,     1.1, 1.1, 0.0,    -25.0};//pt3-4.5 m_lambda==-25.0
  //pt4.5-6.5
  //double paramsupper[8] = {0.4,    1.0,     4.1, 4.1, 1.0,     25.0};
  //double paramslower[8] = {0.01,   0.0,     1.1, 1.1, 0.0,    -25.0};//pt3-4.5 m_lambda==-25.0
  //Cent.10-20
  ...
  //pt6.5-7
  double paramsupper[8] = {0.4,    1.0,     4.1, 4.1, 1.0,     25.0};
  double paramslower[8] = {0.01,   0.0,     1.01, 1.01, 0.0,    -25.0};//pt3-4.5 m_lambda==-25.0
  ...
  double n_1_init;
  //double n_1_init = 1.1;
  if(ptLow==3&&ptHigh==4.5){n_1_init = 1.1;}
  if(ptLow==4.5&&ptHigh==6.5){n_1_init = 3.1;}
  if(ptLow==6.5&&ptHigh==7){n_1_init = 2.1;}
  //double n_1_init = 1.5; //Cent.10-20
  double f_init = 0.5;
  ...
  RooRealVar *sl1 = new RooRealVar("sl1","sl1", .1, -10., 10.);
  RooRealVar *sl2 = new RooRealVar("sl2","sl2", .5, -10., 10.);
  RooRealVar *sl3 = new RooRealVar("sl3","sl3", 1., -10., 10.);
  ...
  RooFitResult* fitMass = ws->pdf("pdfMASS_Tot")->fitTo(*dsAB,Save(), Hesse(kTRUE), Range(massLow,massHigh), Timer(kTRUE), Extended(kTRUE));
  ...
  TFile* outFile;
  if(PR==2){
    outFile = new TFile(Form("../../roots/2DFit_20210208/Mass/MassFitResult_%s_%s.root", bCont.Data(), kineLabel.Data()),"recreate");
    c_A->SaveAs(Form("../../figs/2DFit_20210208/Mass/Mass_%s_%s.pdf", bCont.Data(), kineLabel.Data()));}
  pdfMASS_Tot->Write();
  datasetMass->Write();
  outFile->Close();
