    ...
    )
{
  ...
  gSystem->mkdir("../../roots/2DFit_20210208/", kTRUE);
  gSystem->mkdir("../../figs/2DFit_20210208/", kTRUE);
  ...
  fCTrue = new TFile(Form("../../roots/2DFit_20210208/CtauTrue/CtauTrueResult_%s_%s.root",bCont.Data(),kineLabel.Data()));
  ...
  //ws->factory(Form("AddModel::%s({%s , %s}, %s)", "pdfCTAUCOND_JpsiNoPR", "pdfCTAUTRUE_test1", "pdfCTAUTRUE_test2", "fDSS1"));//}
  ...
  else if(ptLow==6.5&&ptHigh==8.5){ws->factory("b_Jpsi[0.19, 0., 1.]");}//NP fraction for Sig
  ...
  c_G->Update();
  c_G->SaveAs(Form("../../figs/2DFit_20210208/Ctau_%s_%s.pdf",bCont.Data(),kineLabel.Data()));
  c_H->Update();
  c_H->SaveAs(Form("../../figs/2DFit_20210208/Mass_%s_%s.pdf",bCont.Data(),kineLabel.Data()));
  TFile *outFile = new TFile(Form("../../roots/2DFit_20210208/2DFitResult_%s_%s.root",bCont.Data(),kineLabel.Data()),"recreate");
  ws->Write();
  outFile->Close();
}
