    ...
    )
{
  ...
  f1 = new TFile(Form("../../../../skimmedFiles/v2Cut/OniaRooDataSet_isMC0_JPsi_%s_m2.6-3.5_OS_Effw%d_Accw%d_PtW%d_TnP%d_%s_220113.root",kineLabel.Data(),fEffW,fAccW,isPtW,isTnP,DATE.Data()));
  ...
  TFile * f_fit = new TFile(Form("../../Mass/MassFitResult_%s_PRw_Effw%d_Accw%d_PtW%d_TnP%d_test.root",kineLabel.Data(),fEffW,fAccW,isPtW,isTnP));
  ...
  RooRealVar *One = new RooRealVar("One","One", 1.0); // 15<pt<50 v2==-1.2 : 0.01
  RooRealVar *lambda1 = new RooRealVar("lambda1","lambda1", 0.5, 0., 1.);
  RooRealVar *lambda2 = new RooRealVar("lambda2","lambda2", 0.5, 0., 1.);
  RooRealVar *lambda3 = new RooRealVar("lambda3","lambda3", 0.5, 0., 1.);
  RooFormulaVar invMassNorm("invMassNorm","(-1.0+2.0*(@0-@1)/(@2-@1))", RooArgList(*(ws->var("mass")),*mMin,*mMax) );
  //RooFormulaVar pdfMASSPol_Bkg("pdfMASSPol_Bkg","@1*(@0)+1.0",RooArgList(invMassNorm, *lambda1) );
  //RooFormulaVar pdfMASSPol_Bkg("pdfMASSPol_Bkg","@2*(2.0*@0*@0 - 1.0) + @1*(@0) + 1.0",RooArgList(invMassNorm,lambda1,lambda2) );
  //RooFormulaVar pdfMASSPol_Bkg("pdfMASSPol_Bkg","@3*(4.0*@0*@0*@0 - 3.0*@0) + @2*(2.0*@0*@0 - 1.0) + @1*(@0) + 1.0",
  //    RooArgList(invMassNorm,lambda1,lambda2,lambda3) );

  //ws->factory(Form("Exponential::%s(%s, One[1.0])", "pdfMASS_bkg","pdfMASSPol_bkg"));
  //RooExponential pdfMASS_bkg("pdfMASS_bkg","pdfMASS_bkg",*ws->var("pdfMASSPol_bkg"),*One);

  //RooGenericPdf *pdfMASS_bkg = new RooGenericPdf("pdfMASS_bkg","Background","TMath::Exp(-@3*(-1.0+2.0*(@0-@1)/(@2-@1))+1.0/1.0)",RooArgList(*(ws->var("mass")), *mMin, *mMax, *lambda1));
  ...
  pdfMASS_bkg = new RooGenericPdf("pdfMASS_bkg","Background",
      ...
      RooArgList(*(ws->var("mass")), *mMin, *mMax, *lambda1, *lambda2));
  //pdfMASS_bkg = new RooGenericPdf("pdfMASS_bkg","Background",
  //    "TMath::Exp(@5*(4.0*(-1.0+2.0*(@0-@1)/(@2-@1))*(-1.0+2.0*(@0-@1)/(@2-@1))*(-1.0+2.0*(@0-@1)/(@2-@1)) - 3.0*(-1.0+2.0*(@0-@1)/(@2-@1))) + @4*(2.0*(-1.0+2.0*(@0-@1)/(@2-@1))*(-1.0+2.0*(@0-@1)/(@2-@1)) - 1.0) + @3*(-1.0+2.0*(@0-@1)/(@2-@1)) + 1.0)",
  ...
  if (ptLow==15&&ptHigh==50)  {
	  ...
	  Double_t NBkg_limit = 1.0e+05;
	  Double_t NJpsi_limit = 10.0e+04; }
  else if (ptLow==7.5&&ptHigh==9)  {
	  Double_t NBkg_limit = 1.0e+04;
	  Double_t NJpsi_limit = 10.0e+03; }
  else if (ptLow==9&&ptHigh==12)  {
	  Double_t NBkg_limit = 2.0e+04;
	  Double_t NJpsi_limit = 10.0e+04; }
  else if (ptLow==4.5&&ptHigh==6.5)  {
	  Double_t NBkg_limit = 2.0e+06;
	  Double_t NJpsi_limit = 1.0e+05; }
  else if (cLow==80&&cHigh==100){
	 Double_t NBkg_limit = 5.0e+04;
	 Double_t NJpsi_limit = 5.0e+05;  }
  ...
  //RooExtendPdf pdfMASSTot_bkg("pdfMASSTot_bkg", "pdfMASSTot_bkg", pdfMASS_bkg, *ws->var("N_bkg"));
  /*RooRealVar *N_Jpsi;
    RooRealVar *N_Bkg;
  //RooRealVar *N_Jpsi= new RooRealVar("N_Jpsi","inclusive Jpsi signals",0,700000);
  ...
  //RooAddPdf* pdfMASS_Tot = new RooAddPdf("pdfMASS_Tot","Jpsi + Bkg",RooArgList(*pdfMASS_Jpsi,*pdfMASSTot_bkg),RooArgList(*N_Jpsi,*N_Bkg));
  ...
  if(v2==2.1||v2==-2.7) drawText(Form("%.1f < p_{T}^{#mu#mu} < %.1f GeV/c, Cent. %d - %d%s, v2:%.2f-%.2f ",ptLow, ptHigh,cLow/2, cHigh/2, "%", v2, v2+0.6),text_x,text_y,text_color,text_size);
  else drawText(Form("%.1f < p_{T}^{#mu#mu} < %.1f GeV/c, Cent. %d - %d%s, v2:%.2f-%.2f ",ptLow, ptHigh,cLow/2, cHigh/2, "%", v2, v2+0.3),text_x,text_y,text_color,text_size);
  if(yLow==0)drawText(Form("|y^{#mu#mu}| < %.1f",yHigh), text_x,text_y-y_diff,text_color,text_size);
  else if(yLow!=0)drawText(Form("%.1f < |y^{#mu#mu}| < %.1f",yLow, yHigh), text_x,text_y-y_diff,text_color,text_size);
  //drawText(Form("Cent. %d - %d%s", cLow/2, cHigh/2, "%"),text_x,text_y-y_diff*2,text_color,text_size);
