...
void doFitJPsi_Data_pt350_y1624_cent20180(
       float ptLow=3, float ptHigh=50,
       ...
       bool fixParameters=0  )
{
  ...
  int   nMassBin  = (massHigh-massLow)*40;

  TFile* f1 = new TFile("../skimmedFiles/OniaRooDataSet_isMC0_JPsi1SW_20200619.root");
  ...
  if (SiMuPtCut>0) kineCut = kineCut + Form(" && (pt1>%.2f) && (pt2>%.2f)", (float)SiMuPtCut, (float)SiMuPtCut );
  ...
  RooRealVar mean1s("m_{J/#Psi}","mean of the signal gaussian mass PDF",pdgMass.JPsi, pdgMass.JPsi-0.1, pdgMass.JPsi+ 0.1 ) ;
         
  int collId = kAADATA; 
  PSetUpsAndBkg initPset = getUpsilonPsets( collId, ptLow, ptHigh, yLow, yHigh, cLow, cHigh, SiMuPtCut) ; 
  initPset.SetMCSgl();

  RooRealVar    sigma1s_1("sigma1s_1","width/sigma of the signal gaussian mass DF",0.01, 0.01, 0.3);
...
//  RooFormulaVar sigma3s_2("sigma3s_2","@0*@1",RooArgList(sigma1s_2,mRatio31) );
  ...
  RooRealVar alpha1s_1("alpha1s_1","tail shift", 2.522 , 1.166, 4.931);
...
//  RooFormulaVar alpha3s_2("alpha3s_2","1.0*@0",RooArgList(alpha1s_1) );
  ...
  RooRealVar n1s_1("n1s_1","power order", 3.405 , 1.002, 4.845);
...
//  RooRealVar *nSig3s= new RooRealVar("nSig3s"," 3S signals",-100,26000);
  ...
  model = new RooAddPdf("model","1S + Bkg",RooArgList(*cb1s, *bkg),RooArgList(*nSig1s,*nBkg));
...
//  ws->pdf("model")->plotOn(myPlot2,Components(RooArgSet(*cb3s)),LineColor(kOrange+7),LineWidth(2),LineStyle(2));
  ...
  if(collId != kPPDATA && collId != kPPMCUps1S && collId != kPPMCUps2S) 
  {
    ...
    drawText(Form("p_{T}^{#mu} > %.f GeV/c", SiMuPtCut ), pos_text_x,pos_text_y-pos_y_diff*2,text_color,text_size);
    drawText("|#eta^{#mu}| < 2.4 GeV/c", pos_text_x,pos_text_y-pos_y_diff*3,text_color,text_size);
  }  
...
//  cout << "3S signal    =  " << outh->GetBinContent(3) << " +/- " << outh->GetBinError(3) << endl;
  ...
  TFile* outf = new TFile(Form("fitRes/fitresults_jpsi_DoubleCB_%s.root",kineLabel.Data()),"recreate");
  outh->Write();
  c1->SaveAs(Form("fitRes/fitresults_jpsi_DoubleCB_%s.pdf",kineLabel.Data()));
  c1->Write();
  ws->Write();
  ...
  cout <<  ws->var("n1s_1")->getVal() << ", " <<  ws->var("alpha1s_1")->getVal() << ", "<<  ws->var("sigma1s_1")->getVal() << ", " <<  ws->var("m_{J/#Psi}")->getVal() << ", " <<  ws->var("f1s")->getVal() << ", "<<  ws->var("x1s")->getVal() << " );} " << endl;
//  outf->Close();
