...
void get_Eff_pt_jpsi_pbpb_drawsame(
  ...
  //for(int iev=0; iev<300000 ; ++iev)
  {
    ...
    for (Int_t irqq=0; irqq<Reco_QQ_size; ++irqq) 
    {
      ...
      if(! (JP_Reco->Pt()>3 && fabs(JP_Reco->Rapidity())<2.4 && JP_Reco->Pt()<50 && IsAcceptanceQQ(mupl_Reco->Pt(),fabs(mupl_Reco->Eta()))&&IsAcceptanceQQ(mumi_Reco->Pt(),fabs(mumi_Reco->Eta()))) ) continue;
...
//         cout << "TnP ERROR !!!! ::: No matched L2 filter1 " << endl;
      ...
      if(HLTPass==true && HLTFilterPass==true){
	      ...
	      if(Rapidity < 0.6) hpt_reco_2 -> Fill(JP_Reco->Pt(), weight* tnp_weight* pt_weight);
	      else if(Rapidity > 0.6 && Rapidity < 1.2) hpt_reco_3 -> Fill(JP_Reco->Pt(), weight* tnp_weight* pt_weight);
	      else if(Rapidity > 1.2 && Rapidity < 1.8) hpt_reco_4 -> Fill(JP_Reco->Pt(), weight* tnp_weight* pt_weight);
	      else if(Rapidity > 1.8 && Rapidity < 2.4) hpt_reco_5 -> Fill(JP_Reco->Pt(), weight* tnp_weight* pt_weight);
      }
    }
  }
...
//draw same
  ...
  hpt_eff_5->Draw("same");
  legend->AddEntry("hpt_eff_1","|y|: 0-2.4, 50-90%","lep");
  legend->AddEntry("hpt_eff_2","|y|: 0-0.6, 50-90%","lep");
  legend->AddEntry("hpt_eff_3","|y|: 0.6-1.2, 50-90%","lep");
  legend->AddEntry("hpt_eff_4","|y|: 1.2-1.8, 50-90%","lep");
  legend->AddEntry("hpt_eff_5","|y|: 1.8-2.4, 50-90%","lep");
  ...
  hpt_eff_1 ->SetName(Form("mc_eff_vs_pt_TnP%d_PtW%d_cent100to180_absy0_2p4",isTnP, isPtWeight));
  hpt_eff_2 ->SetName(Form("mc_eff_vs_pt_TnP%d_PtW%d_cent100to180_absy0_0p6  ",isTnP, isPtWeight));
  hpt_eff_3 ->SetName(Form("mc_eff_vs_pt_TnP%d_PtW%d_cent100to180_absy0p6_1p2",isTnP, isPtWeight));
  hpt_eff_4 ->SetName(Form("mc_eff_vs_pt_TnP%d_PtW%d_cent100to180_absy1p2_1p6",isTnP, isPtWeight));
  hpt_eff_5 ->SetName(Form("mc_eff_vs_pt_TnP%d_PtW%d_cent100to180_absy1p6_2p4",isTnP, isPtWeight));
  hy_eff ->SetName(Form("mc_eff_vs_rap_TnP%d_PtW%d",isTnP, isPtWeight));

  TString outFileName = Form("mc_eff_vs_pt_cent100to180_rap_prompt_pbpb_Jpsi_PtW%d_tnp%d_drawsame.root",isPtWeight,isTnP);
  if(state==2) outFileName = Form("mc_eff_vs_pt_cent100to180_rap_nprompt_pbpb_Jpsi_PtW%d_tnp%d_drawsame.root",isPtWeight,isTnP);
