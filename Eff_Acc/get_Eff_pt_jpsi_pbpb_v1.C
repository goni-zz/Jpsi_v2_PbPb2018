...
void get_Eff_pt_jpsi_pbpb_v1(
  float ptLow = 0.0, float ptHigh = 50.0,
  float yLow = 0.0, float yHigh = 2.4,
  float cLow = 100, float cHigh = 180, 
  ...
  //for(int iev=0; iev<300000 ; ++iev)
  {
    ...
    for(int igen = 0; igen<Gen_QQ_size; igen++){
	    ...
	    else if(Rapidity_g > 0.6 && Rapidity_g < 1.2 && JP_Gen->Pt() > 6.5) hpt_gen_3 -> Fill(JP_Gen->Pt(), weight*pt_weight);
...
//         cout << "TnP ERROR !!!! ::: No matched L2 filter1 " << endl;
      ...
      if(HLTPass==true && HLTFilterPass==true){
	      ...
	      else if(Rapidity > 0.6 && Rapidity < 1.2 && JP_Reco->Pt() > 6.5) hpt_reco_3 -> Fill(JP_Reco->Pt(), weight* tnp_weight* pt_weight);
...
//draw same
  ...
  legend->AddEntry("hpt_eff_3",Form("|y|: 0.6-1.2, %0.0f-%0.0f %%",cLow/2,cHigh/2),"lep");
  ...
  legend->Draw("E");
  if(state==1) cpt_eff->SaveAs(Form("Eff_pt_P_noweight2_cent_%0.0f_to_%0.0f.png", cLow, cHigh));
  else if(state==2) cpt_eff->SaveAs(Form("Eff_pt_NP_noweight2_cent_%0.0f_to_%0.0f.png", cLow, cHigh));
  ...
  if(state==1) cy_eff->SaveAs(Form("Eff_absy_P_noweight2_cent_%0.0f_to_%0.0f.png", cLow, cHigh));
  else if(state==2) cy_eff->SaveAs(Form("Eff_absy_NP_noweight2_cent_%0.0f_to_%0.0f.png", cLow, cHigh));
  ...
  hpt_eff_3 ->SetName(Form("mc_eff_vs_pt_TnP%d_PtW%d_cent_%0.0f_to_%0.0f_absy0p6_1p2",isTnP, isPtWeight, cLow, cHigh));
  ...
  TString outFileName = Form("mc_eff_vs_pt_cent_%0.0f_to_%0.0f_rap_prompt_pbpb_Jpsi_PtW%d_tnp%d.root",cLow,cHigh,isPtWeight,isTnP);
  if(state==2) outFileName = Form("mc_eff_vs_pt_cent_%0.0f_to_%0.0f_rap_nprompt_pbpb_Jpsi_PtW%d_tnp%d.root",cLow,cHigh,isPtWeight,isTnP);
