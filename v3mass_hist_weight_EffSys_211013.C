...
void v3mass_hist_weight_EffSys_211013(
    ...
    ) //PR 0: PR, 1: NP, 2: Inc.
{
  ...
  gSystem->mkdir(Form("figs/EffSys_211013/v3mass_hist/%s",DATE.Data()), kTRUE);
  gSystem->mkdir(Form("figs/EffSys_211013/v3q_vector/%s",DATE.Data()), kTRUE);
  gSystem->mkdir(Form("figs/EffSys_211013/v3mass_dist/%s",DATE.Data()), kTRUE);
  gSystem->mkdir(Form("figs/EffSys_211013/v3decayL/%s",DATE.Data()), kTRUE);
  gSystem->mkdir(Form("roots/EffSys_211013/v3mass_hist/%s",DATE.Data()), kTRUE);
  ...
  if(!isMC){
    ...
    TString f1 = "skimmedFiles/OniaFlowSkim_JpsiTrig_DBAllPD_isMC0_HFNom_v3_210531.root";//Nominal
  ...
  TFile *fEff;
  if(ptLow==6.5&&ptHigh==50) fEff= new TFile(Form("./Eff_Acc/roots/mc_eff_vs_pt_cent_0_to_180_rap_%s_pbpb_Jpsi_PtW%d_tnp%d_210913.root",wName.Data(),0,isTnP),"read");
  else fEff= new TFile(Form("./Eff_Acc/roots/mc_eff_vs_pt_cent_%i_to_%i_rap_%s_pbpb_Jpsi_PtW%d_tnp%d_210913.root",cLow,cHigh,wName.Data(),0,isTnP),"read");

  TH1D* hEffPt[4];
  if(ptLow==6.5&&ptHigh==50){
    hEffPt[0] = (TH1D*) fEff -> Get(Form("mc_eff_vs_pt_TnP%d_PtW%d_cent_0_to_180_absy0_1p2",isTnP,0));
    hEffPt[1] = (TH1D*) fEff -> Get(Form("mc_eff_vs_pt_TnP%d_PtW%d_cent_0_to_180_absy1p2_1p6",isTnP,0));
    hEffPt[2] = (TH1D*) fEff -> Get(Form("mc_eff_vs_pt_TnP%d_PtW%d_cent_0_to_180_absy1p6_1p8",isTnP,0));
    hEffPt[3] = (TH1D*) fEff -> Get(Form("mc_eff_vs_pt_TnP%d_PtW%d_cent_0_to_180_absy1p8_2p4",isTnP,0));}
  else{
    hEffPt[0] = (TH1D*) fEff -> Get(Form("mc_eff_vs_pt_TnP%d_PtW%d_cent_%i_to_%i_absy0_1p2",isTnP,0,cLow,cHigh));
    hEffPt[1] = (TH1D*) fEff -> Get(Form("mc_eff_vs_pt_TnP%d_PtW%d_cent_%i_to_%i_absy1p2_1p6",isTnP,0,cLow,cHigh));
    hEffPt[2] = (TH1D*) fEff -> Get(Form("mc_eff_vs_pt_TnP%d_PtW%d_cent_%i_to_%i_absy1p6_1p8",isTnP,0,cLow,cHigh));
    hEffPt[3] = (TH1D*) fEff -> Get(Form("mc_eff_vs_pt_TnP%d_PtW%d_cent_%i_to_%i_absy1p8_2p4",isTnP,0,cLow,cHigh));}
  ...
  //fCErr = new TFile(Form("Macros/2021_09_14/roots/EffSys_211013/2DFit_%s/CtauErr/CtauErrResult_%s_%sw_Effw%d_Accw%d_PtW%d_TnP%d.root", DATE.Data(), kineLabel.Data(), "PR", fEffW, fAccW, isPtW, isTnP));
  ...
  pad1->Draw();
  pad2->Draw();
  //gSystem->mkdir(Form("figs/EffSys_211013/v3q_vector",sample.Data()),1);
  if(ctauCut==0) c_mass_v3->SaveAs(Form("figs/EffSys_211013/v3mass_hist/%s/v3Mass_%s_ctau_%.5f_%.5f_%s.pdf",DATE.Data(),kineLabel.Data(),ctauLo,ctauHi,cutName.Data()));
  else if(ctauCut==-1)c_mass_v3->SaveAs(Form("figs/EffSys_211013/v3mass_hist/%s/v3Mass_%s_ctau_%.5f_%s.pdf",DATE.Data(),kineLabel.Data(),ctauLo,cutName.Data()));
  else if(ctauCut== 1)c_mass_v3->SaveAs(Form("figs/EffSys_211013/v3mass_hist/%s/v3Mass_%s_ctau_%.5f_%s.pdf",DATE.Data(),kineLabel.Data(),ctauHi,cutName.Data()));
  else c_mass_v3->SaveAs(Form("figs/EffSys_211013/v3mass_hist/%s/v3Mass_%s_Inc.pdf",DATE.Data(),kineLabel.Data()));
  ...
  //lcut->Draw("same");
  c_pt->Update();
  if(ctauCut==0) c_pt->SaveAs(Form("figs/EffSys_211013/pt/%s/pt_%s_ctau_%.5f_%.5f_%s.pdf",DATE.Data(),kineLabel.Data(),ctauLo,ctauHi,cutName.Data()));
  else if(ctauCut==-1)c_pt->SaveAs(Form("figs/EffSys_211013/pt/%s/pt_%s_ctau_%.5f_%s.pdf",DATE.Data(),kineLabel.Data(),ctauLo,cutName.Data()));
  else if(ctauCut== 1)c_pt->SaveAs(Form("figs/EffSys_211013/pt/%s/pt_%s_ctau_%.5f_%s.pdf",DATE.Data(),kineLabel.Data(),ctauHi,cutName.Data()));
  else c_pt->SaveAs(Form("figs/EffSys_211013/pt/%s/pt_%s_Inc.pdf",DATE.Data(),kineLabel.Data()));
  ...
  //lcut->Draw("same");
  c_decayL->Update();
  if(ctauCut==0) c_decayL->SaveAs(Form("figs/EffSys_211013/v3decayL/%s/decayL_%s_ctau_%.5f_%.5f_%s.pdf",DATE.Data(),kineLabel.Data(),ctauLo,ctauHi,cutName.Data()));
  else if(ctauCut==-1)c_decayL->SaveAs(Form("figs/EffSys_211013/v3decayL/%s/decayL_%s_ctau_%.5f_%s.pdf",DATE.Data(),kineLabel.Data(),ctauLo,cutName.Data()));
  else if(ctauCut== 1)c_decayL->SaveAs(Form("figs/EffSys_211013/v3decayL/%s/decayL_%s_ctau_%.5f_%s.pdf",DATE.Data(),kineLabel.Data(),ctauHi,cutName.Data()));
  else c_decayL->SaveAs(Form("figs/EffSys_211013/v3decayL/%s/decayL_%s_Inc.pdf",DATE.Data(),kineLabel.Data()));
 ...
  CMS_lumi_v2mass(c_mass,iPeriod,iPos);
  c_mass->Update();
  if(ctauCut==0)c_mass->SaveAs(Form("figs/EffSys_211013/v3mass_dist/%s/MassDist_%s_ctau_%.5f_%.5f_%s.pdf",DATE.Data(),kineLabel.Data(),ctauLo,ctauHi,cutName.Data()));
  else if(ctauCut==-1)c_mass->SaveAs(Form("figs/EffSys_211013/v3mass_dist/%s/MassDist_%s_ctau_%.5f_%s.pdf",DATE.Data(),kineLabel.Data(),ctauLo,cutName.Data()));
  else if(ctauCut==1)c_mass->SaveAs(Form("figs/EffSys_211013/v3mass_dist/%s/MassDist_%s_ctau_%.5f_%s.pdf",DATE.Data(),kineLabel.Data(),ctauHi,cutName.Data()));
  else c_mass->SaveAs(Form("figs/EffSys_211013/v3mass_dist/%s/MassDist_%s_Inc.pdf",DATE.Data(),kineLabel.Data()));
  ...
  if(ctauCut==0) c_qq_1->SaveAs(Form("figs/EffSys_211013/v3q_vector/%s/c_qqa_%s_Eff%d_Acc%d_PtW%d_TnP%d_ctau_%.5f_%.5f_%s.pdf",DATE.Data(),kineLabel.Data(),fEffW,fAccW,isPtW,isTnP,ctauLo,ctauHi,cutName.Data()));
  else if(ctauCut==-1)c_qq_1->SaveAs(Form("figs/EffSys_211013/v3q_vector/%s/c_qqa_%s_Eff%d_Acc%d_PtW%d_TnP%d_ctau_%.5f_%s.pdf",DATE.Data(),kineLabel.Data(),fEffW,fAccW,isPtW,isTnP,ctauLo,cutName.Data()));
  else if(ctauCut==1)c_qq_1->SaveAs(Form("figs/EffSys_211013/v3q_vector/%s/c_qqa_%s_Eff%d_Acc%d_PtW%d_TnP%d_ctau_%.5f_%s.pdf",DATE.Data(),kineLabel.Data(),fEffW,fAccW,isPtW,isTnP,ctauHi,cutName.Data()));
  else c_qq_1->SaveAs(Form("figs/EffSys_211013/v3q_vector/%s/c_qqa_%s_Eff%d_Acc%d_PtW%d_TnP%d_Inc.pdf",DATE.Data(),kineLabel.Data(),fEffW,fAccW,isPtW,isTnP));
  ...
  leg_v3_2->Draw("same");
  if(ctauCut==0) c_qq_2->SaveAs(Form("figs/EffSys_211013/v3q_vector/%s/c_qaqb_%s_Eff%d_Acc%d_PtW%d_TnP%d_ctau_%.5f_%.5f_%s.pdf",DATE.Data(),kineLabel.Data(),fEffW,fAccW,isPtW,isTnP,ctauLo,ctauHi,cutName.Data()));
  else if(ctauCut==-1)c_qq_2->SaveAs(Form("figs/EffSys_211013/v3q_vector/%s/c_qaqb_%s_Eff%d_Acc%d_PtW%d_TnP%d_ctau_%.5f_%s.pdf",DATE.Data(),kineLabel.Data(),fEffW,fAccW,isPtW,isTnP,ctauLo,cutName.Data()));
  else if(ctauCut==1)c_qq_2->SaveAs(Form("figs/EffSys_211013/v3q_vector/%s/c_qaqb_%s_Eff%d_Acc%d_PtW%d_TnP%d_ctau_%.5f_%s.pdf",DATE.Data(),kineLabel.Data(),fEffW,fAccW,isPtW,isTnP,ctauHi,cutName.Data()));
  else c_qq_2->SaveAs(Form("figs/EffSys_211013/v3q_vector/%s/c_qaqb_%s_Eff%d_Acc%d_PtW%d_TnP%d_Inc.pdf",DATE.Data(),kineLabel.Data(),fEffW,fAccW,isPtW,isTnP));
  ...
  leg_v3_3->Draw("same");
  if(ctauCut==0) c_qq_3->SaveAs(Form("figs/EffSys_211013/v3q_vector/%s/c_qaqc_%s_Eff%d_Acc%d_PtW%d_TnP%d_ctau_%.5f_%.5f_%s.pdf",DATE.Data(),kineLabel.Data(),fEffW,fAccW,isPtW,isTnP,ctauLo,ctauHi,cutName.Data()));
  else if(ctauCut==-1)c_qq_3->SaveAs(Form("figs/EffSys_211013/v3q_vector/%s/c_qaqc_%s_Eff%d_Acc%d_PtW%d_TnP%d_ctau_%.5f_%s.pdf",DATE.Data(),kineLabel.Data(),fEffW,fAccW,isPtW,isTnP,ctauLo,cutName.Data()));
  else if(ctauCut== 1)c_qq_3->SaveAs(Form("figs/EffSys_211013/v3q_vector/%s/c_qaqc_%s_Eff%d_Acc%d_PtW%d_TnP%d_ctau_%.5f_%s.pdf",DATE.Data(),kineLabel.Data(),fEffW,fAccW,isPtW,isTnP,ctauHi,cutName.Data()));
  else c_qq_3->SaveAs(Form("figs/EffSys_211013/v3q_vector/%s/c_qaqc_%s_Eff%d_Acc%d_PtW%d_TnP%d_Inc.pdf",DATE.Data(),kineLabel.Data(),fEffW,fAccW,isPtW,isTnP));
  ...
  leg_v3_4->Draw("same");
  if(ctauCut==0) c_qq_4->SaveAs(Form("figs/EffSys_211013/v3q_vector/%s/c_qbqc_%s_Eff%d_Acc%d_PtW%d_TnP%d_ctau_%.5f_%.5f_%s.pdf",DATE.Data(),kineLabel.Data(),fEffW,fAccW,isPtW,isTnP,ctauLo,ctauHi,cutName.Data()));
  else if(ctauCut==-1)c_qq_4->SaveAs(Form("figs/EffSys_211013/v3q_vector/%s/c_qbqc_%s_Eff%d_Acc%d_PtW%d_TnP%d_ctau_%.5f_%s.pdf",DATE.Data(),kineLabel.Data(),fEffW,fAccW,isPtW,isTnP,ctauLo,cutName.Data()));
  else if(ctauCut==1)c_qq_4->SaveAs(Form("figs/EffSys_211013/v3q_vector/%s/c_qbqc_%s_Eff%d_Acc%d_PtW%d_TnP%d_ctau_%.5f_%s.pdf",DATE.Data(),kineLabel.Data(),fEffW,fAccW,isPtW,isTnP,ctauHi,cutName.Data()));
  else c_qq_4->SaveAs(Form("figs/EffSys_211013/v3q_vector/%s/c_qbqc_%s_Eff%d_Acc%d_PtW%d_TnP%d_Inc.pdf",DATE.Data(),kineLabel.Data(),fEffW,fAccW,isPtW,isTnP));

  TFile *wf; 
  if(ctauCut==0) wf = new TFile(Form("roots/EffSys_211013/v3mass_hist/%s/Jpsi_%s_Eff%d_Acc%d_PtW%d_TnP%d_ctau_%.5f_%.5f_%s.root",DATE.Data(),kineLabel.Data(),fEffW,fAccW,isPtW,isTnP,ctauLo,ctauHi,cutName.Data()),"recreate");
  else if(ctauCut==-1) wf = new TFile(Form("roots/EffSys_211013/v3mass_hist/%s/Jpsi_%s_Eff%d_Acc%d_PtW%d_TnP%d_ctau_%.5f_%s.root",DATE.Data(),kineLabel.Data(),fEffW,fAccW,isPtW,isTnP,ctauLo,cutName.Data()),"recreate");
  else if(ctauCut==1) wf = new TFile(Form("roots/EffSys_211013/v3mass_hist/%s/Jpsi_%s_Eff%d_Acc%d_PtW%d_TnP%d_ctau_%.5f_%s.root",DATE.Data(),kineLabel.Data(),fEffW,fAccW,isPtW,isTnP,ctauHi,cutName.Data()),"recreate");
  else wf = new TFile(Form("roots/EffSys_211013/v3mass_hist/%s/Jpsi_%s_Eff%d_Acc%d_PtW%d_TnP%d_Inc.root",DATE.Data(),kineLabel.Data(),fEffW,fAccW,isPtW,isTnP),"recreate");
  wf->cd();
  h_v3_final->Write();
  h_decayL->Write();
  g_mass->Write();
