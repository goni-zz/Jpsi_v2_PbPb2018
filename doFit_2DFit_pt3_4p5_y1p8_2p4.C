...
void doFit_2DFit_pt3_4p5_y1p8_2p4( 
			...
			) 
{
  ...
  //Select Data Set
    ...
    f2 = new TFile("skimmedFiles/OniaRooDataSet_NonPrompt_GenInReco_3_6p5_201005.root");
    f3 = new TFile("skimmedFiles/OniaRooDataSet_NonPrompt_GenInReco_6p5_50_201005.root");
  ...
  RooDataSet *datasetMC1 = (RooDataSet*)f2->Get("dataset");
...
//A: Mass Bkg function
  ...
  //RooChebychev *bkg;
  //bkg = new RooChebychev("bkg","Background",*(ws->var("mass")),RooArgList(*sl1,*cnst1));
  //Build the model
//Model A: Mass
  ...
  RooFitResult* fitMass = ws->pdf("pdfMASS_Tot")->fitTo(*dsAB,Save(), Hesse(kTRUE), Range(massLow,massHigh), Timer(kTRUE), Extended(kTRUE), NumCPU(12), PrintLevel(-1));
...
// create the variables for this model
  ...
  ws->factory("ctau3_CtauRes[0., -0.1, 0.1]");  ws->factory("s3_CtauRes[3,  1e-6, 5.]");
...
//MC NP ctau true
  ...
  pad_D_1->cd();
  gPad->SetLogy();
  RooFitResult* fitCtauTrue = ws->pdf("TrueModel_Tot")->fitTo(*reducedDS_MC, SumW2Error(false), Range("ctauTrueRange"), Extended(kTRUE), NumCPU(12), PrintLevel(3), Save());
